...
class GraphicsMethodGui:
    ...
    def __init__(self, vcs=None, gm_type='boxfill', gm_name='default',
        ...
        if (gm_type not in graphicsmethodlist()):
            ...
            str = "The %s graphics method name (%s) does not exist!" % (
                gm_type2, gm_name)
            showerror("Error Message to User", str)
        ...
        if (gm_type == 'boxfill'):
            ...
            gui_support.balloon.bind(
                notebook.tab('Boxfill Settings'),
                'The Boxfill graphics method displays a two-dimensional data\narray by surrounding each data value with a colored grid box.')
##            gm = self.set_gm_entries(gm_type, gm_name, vcs )
            gm = vcs.getboxfill(gm_name)
            self.GM = self.boxfill_attributes = browser.gui_set_graphics_methods.boxfill_attributes(
                page,
                gui_parent_updated,
                re_plot_flag=0,
                gm=gm)
...
##            gm = self.set_gm_entries(gm_type, gm_name, vcs )
            ...
            gui_support.balloon.bind(
                notebook.tab('Meshfill Settings'),
                "The Meshfill graphics method draws data on irregular grid (or 'mesh')at specified levels to graphically represent\nthe values of a one-dimensional array;\nUnless the irregular grid is supported by cdms, a mesh array must be passed as well")
            gm = vcs.getmeshfill(gm_name)
            self.GM = self.meshfill_attributes = browser.gui_set_graphics_methods.meshfill_attributes(
                page,
                gui_parent_updated,
                re_plot_flag=0,
                gm=gm)
...
##               gm = self.set_gm_entries(gm_type, gm_name, vcs )
                ...
                gui_support.balloon.bind(
                    notebook.tab(contour_type),
                    "The primary purpose of the Outline graphics method\nis to display outlined continents or sea ice using\na surface type array that indicates land, ocean,\nand sea ice points. In general, however, this\ngraphics method can be used to outline a set of\ninteger values for any array. ")
##               gm = self.set_gm_entries(gm_type, gm_name, vcs )
                self.GM = self.outline_attributes = browser.gui_set_graphics_methods.outline_attributes(
                    page,
                    gui_parent_updated,
                    re_plot_flag=0,
                    gm=gm)
            ...
            if gm_type == 'xvsy':
                gui_support.balloon.bind(
                    notebook.tab(oneD_type),
                    "The XvsY graphics method displays a line plot from\ntwo 1D data arrays, that is X(t) and Y(t), where t\nrepresents the 1D coordinate values.")
            elif gm_type == 'xyvsy':
                gui_support.balloon.bind(
                    notebook.tab(oneD_type),
                    "The Xyvsy graphics method displays a line plot from\na 1D data array, that is X(y), where y represents\nthe 1D coordinate values.")
            elif gm_type == 'yxvsx':
                gui_support.balloon.bind(
                    notebook.tab(oneD_type),
                    "The Yxvsx graphics method displays a line plot from\na 1D data array, that is Y(x), where y represents\nthe 1D coordinate values.")

##            gm = self.set_gm_entries(gm_type, gm_name, vcs )
            self.GM = self.oneD_attributes = browser.gui_set_graphics_methods.oneD_attributes(
                page,
                gui_parent_updated,
                re_plot_flag=0,
                gm=gm)
            ...
            gui_support.balloon.bind(
                notebook.tab('Scatter Settings'),
                "The Scatter graphics method displays a a scatter\nplot of two data arrays A(x, y, z, t) and B(x, y,\nz, t). ")
            gm = vcs.getscatter(gm_name)
##            gm = self.set_gm_entries(gm_type, gm_name, vcs )
            self.GM = self.scatter_attributes = browser.gui_set_graphics_methods.scatter_attributes(
                page,
                gui_parent_updated,
                re_plot_flag=0,
                gm=gm)
...
##            gm = self.set_gm_entries(gm_type, gm_name, vcs )
            ...
            gui_support.balloon.bind(
                notebook.tab('Continents Settings'),
                "The primary purpose of the Continents graphics method\nis to display a predefined, generic set of continental\noutlines in a longitude x latitude space.")
            gm = vcs.getcontinents(gm_name)
##            gm = self.set_gm_entries(gm_type, gm_name, vcs )
            self.GM = self.continents_attributes = browser.gui_set_graphics_methods.continents_attributes(
                page,
                gui_parent_updated,
                re_plot_flag=0,
                gm=gm)
...
##         self.store_gm_xmtics2 = self.gm.xmtics2
    ...
    def apply(self, gui_parent):
        ...
        # Automatic update of the display if necessary. Only update if it is
        # called from VCDAT.
        if gui_parent is not None:
...
#               dfj = self.vcs.getisofill('ASD')
                ...
                if (gui_parent.pl.form[i].gm.get() == self.gm.name) and (
                        gm_type == fm_label):
...
def create_graphicsmethod_file_menu(main_menu, eself, gui_parent):
    ...
    main_menu.addmenuitem('File', 'command', 'Save under new name',
                          label='Rename %s Graphics Method to...' % string.capitalize(
                              eself.gm_type),
                          command=G_Command(
                              evt_rename_graphics_methods,
                              eself,
                              gui_parent)
                          )
    #
    ...
    main_menu.addmenuitem('File', 'command', 'Save to script file',
                          label='Save %s as a Script File...' % string.capitalize(
                              eself.gm_type),
                          command=G_Command(
                              evt_savescript_graphics_methods,
                              eself,
                              gui_parent)
                          )
...
def evt_new_graphics_methods(eself, gui_parent):
    ...
    hold.dialog = dialog = Pmw.PromptDialog(eself.root,
                                            title='Create new %s graphics method from %s' % (
                                                eself.gm_type, eself.gm_name),
                                            label_text='Enter New %s Name:' % string.capitalize(
                                                eself.gm_type),
    ...
    # draw widget on top of its parent
    dialog.transient(eself.dialog.interior())
...
def execute_new_selection(eself, gui_parent, hold, result):
    ...
    if result == 'OK':
        ...
        if new_name in eself.vcs.listelements(gm_type):
            gui_message.error(
                'Error occurred while trying to create the new %s graphics method %s from %s. Make sure the new graphics method name does not already exist.' %
                (gm_type, new_name, gm_name))
            return
        try:
        ...
        if (gui_parent is not None) and (
                gui_parent.panelDV.gm_name == gm_type):
...
def evt_which_graphics_method(dialog, event):
    gm_type = dialog.text_opt2.get()
    dialog.configure(
        label_text='Enter New %s Name:' %
        string.capitalize(gm_type))


def evt_edit_graphics_methods(eself, gui_parent):
    # Create the popup dialog.
    eself.dialog_tmp = Pmw.ComboBoxDialog(eself.root,
                                          title='Select %s Graphics Method' % string.capitalize(
                                              eself.gm_type),
                                          buttons=('OK', 'Cancel'),
                                          defaultbutton = 'OK',
                                          combobox_labelpos = 'n',
                                          label_text = 'Enter or Select %s Name:' % string.capitalize(
                                              eself.gm_type),
                                          scrolledlist_items = eself.vcs.listelements(
                                              eself.gm_type),
                                          command = G_Command(
                                              execute_load_selection,
                                              eself,
                                              gui_parent)
                                          )
    ...
    opt2 = Pmw.OptionMenu(eself.dialog_tmp.interior(),
                          ...
                          items=gmchlst,
                          command=G_Command(
        evt_which_edit_graphics_method,
        eself,
        eself.dialog_tmp)
    )
    ...
    # draw widget on top of its parent
    eself.dialog_tmp.transient(eself.dialog.interior())
...
def execute_load_selection(eself, gui_parent, result):
    if result == 'OK':
        ...
        if new_name in eself.vcs.listelements(eself.gm_type):
            # Popup new graphics method gui
            eself.vcs.graphicsmethodgui(
                eself.gm_type,
                new_name,
                gui_parent=gui_parent)
        else:
            gui_message.error(
                'Error occurred while trying to load %s graphics method %s. Make sure the graphics method name exist.' %
                (eself.gm_type, new_name))
...
def evt_rename_graphics_methods(eself, gui_parent):
    # Popup use dialog widget
    hold.dialog = dialog = Pmw.PromptDialog(eself.root,
                                            title='Rename %s graphics method from %s to ?' % (
                                                eself.gm_type, eself.gm_name),
                                            label_text='Enter New %s Name:' % string.capitalize(
                                                eself.gm_type),
                                            entryfield_labelpos='nw',
...
def execute_rename_selection(eself, gui_parent, hold, result):
    ...
    if result == 'OK':
        ...
        if new_name in eself.vcs.listelements(eself.gm_type):
            gui_message.error(
                'An error occurred while trying to rename the %s graphics method %s to %s. Make sure the new graphics method name does not already exist.' %
                (eself.gm_type, eself.gm_name, new_name))
            return
        try:
...
Lawrence Livermore National Laboratory Livermore, CA 94550 """)
        self.about = Pmw.AboutDialog(
            parent,
            applicationname='The Visualization Control System - (VCS) Graphics Method Editor')
...
#        btn.pack( expand = 1, padx=5, pady=5 )
    ...
    def set(self):
        try:
            self.gm.xticlabels1 = eval(self.xtic1.get())
        except:
            self.gm.xticlabels1 = self.xtic1.get()
        try:
            self.gm.xticlabels2 = eval(self.xtic2.get())
        except:
            self.gm.xticlabels2 = self.xtic2.get()
        try:
            self.gm.xmtics1 = eval(self.xmtic1.get())
        except:
            self.gm.xmtics1 = self.xmtic1.get()
        try:
            self.gm.xmtics2 = eval(self.xmtic2.get())
        except:
            self.gm.xmtics2 = self.xmtic2.get()
...
#        btn.pack( expand = 1, padx=5, pady=5 )
    ...
    def set(self):
        try:
            self.gm.yticlabels1 = eval(self.ytic1.get())
        except:
            self.gm.yticlabels1 = self.ytic1.get()
        try:
            self.gm.yticlabels2 = eval(self.ytic2.get())
        except:
            self.gm.yticlabels2 = self.ytic2.get()
        try:
            self.gm.ymtics1 = eval(self.ymtic1.get())
        except:
            self.gm.ymtics1 = self.ymtic1.get()
        try:
            self.gm.ymtics2 = eval(self.ymtic2.get())
        except:
            self.gm.ymtics2 = self.ymtic2.get()
...
class wc_attributes:
    ...
    def __init__(self, page, parent, gm):
        ...
        gui_support.balloon.bind(
            self.datawc2,
            "Set new Y1 world coordinate value. If value is 1e+20,\nthen VCS will use the data's coordinate value specified\nin the dimension.")
        ...
        gui_support.balloon.bind(
            self.datawc3,
            "Set new X2 world coordinate value. If value is 1e+20,\nthen VCS will use the data's coordinate value specified\nin the dimension.")
        ...
        gui_support.balloon.bind(
            self.datawc4,
            "Set new Y2 world coordinate value. If value is 1e+20,\nthen VCS will use the data's coordinate value specified\nin the dimension.")
        self.reset()
#        btn=Tkinter.Button(page,
...
class pa_attributes:
    ...
    def __init__(self, page, parent, gm, gm_type, canvas, gui_parent):
        ...
        group1 = Pmw.Group(page, tag_text='X Major Ticks and Labels')
        projections = sorted(canvas.listelements('projection'))
        ...
        self.button = Tkinter.Button(frm,
                                     ...
                                     command=gui_control.Command(
                                         projectiongui.ProjGUI,
                                         canvas=self.canvas,
                                         gui_parent=gui_parent,
                                         dialog_parent=self.parent,
                                         master=frm,
                                         name=self.gm.projection,
                                         Parent=self),
        ...
        gui_support.balloon.bind(
            self.projection,
            "Choose new graphics method projection.")
        ...
        if (gm_type not in ['xyvsy', 'continents']):
            ...
            gui_support.balloon.bind(
                self.xaxisconvert,
                "Choose new X-axis representation.")
        ...
        if (gm_type not in ['yxvsx', 'continents']):
            ...
            gui_support.balloon.bind(
                self.yaxisconvert,
                "Choose new Y-axis representation.")
        self.reset()
#        btn=Tkinter.Button(page,
