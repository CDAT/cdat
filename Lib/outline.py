...
"""
    ...
    def _setoutline(self, value):
        if (type(value) in (ListType, TupleType, IntType, FloatType)):
            ...
            if (len(value) > 10):
                raise ValueError(
                    'The outline attribute must have less than 10 values.')
            else:
                self._outline = value
                setmember(self, 'outline', value)
        else:
            raise ValueError(
                'The outline attribute must be a tuple or list of values.')
    outline = property(_getoutline, _setoutline)

    def __init__(
            self, parent, Go_name=None, Go_name_src='default', createGo=0):
        #                                                         #
        ###########################################################
        ...
        self.__doc__ = self.__doc__ % (
            xmldocs.graphics_method_core, xmldocs.linedoc, xmldocs.outlinerdoc)
...
"""
        ...
        if (scr_type == '.scr'):
            ...
            fp.write(
                "#----------Outline (Go) member (attribute) listings ----------\n")
