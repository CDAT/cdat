...
class AnimationGui:
    ...
    def __init__(self, animation, gui_parent=None, transient=0):
        ...
        if (gui_parent is not None) and (transient == 1):
            # Keep widget on top of its parent
    ...
    def execute(self, parent, event):
        ...
        if (parent is not None):
            if (parent.panelGC.over_flg != 1) or (
                ...
                for i in parent.pl.form.keys():
                    if parent.pl.form[i].lbl.cget('label_text') == 'On':
                        try:
                            ...
                            pass
                        parent.pl.form[i].off_btn.create_image(
                            0,
                            0,
                            anchor=Tkinter.NW,
                            image=parent.pl.form[i].off_btnimage)
...
Lawrence Livermore National Laboratory Livermore, CA 94550 """)
        self.about = Pmw.AboutDialog(
            self.parent,
            applicationname="The Visualization Control System's - (VCS) Animation Editor")
...
#         self.mode_toggle_2.set( 0 )
    ...
    def evt_set_min_max(self, eself, parent, animation):
        ...
        # animation.update_animate_display_list( ) # make sure the list is
        # updated

        if len(animation.vcs_self.animate_info) < 2:
            for text in ('Animation sets min and max',
                         'User sets min and max', 'Turn off animation min and max'):
                self.radiobuttons.add(text)
            self.radiobuttons.invoke('Animation sets min and max')
        else:
            for text in ('Animation sets min and max for all data sets',
                         'User sets min and max for each data set', 'Turn off animation min and max for each data set'):
                self.radiobuttons.add(text)
            self.radiobuttons.invoke(
                'Animation sets min and max for all data sets')

        self.min = Pmw.EntryField(self.mmdialog.interior(),
                                  labelpos='w',
                                  label_text='Min:',
                                  value='0',
                                  modifiedcommand=A_Command(
            self.set_toggle_flg,
            animation)
        )
        ...
        if len(animation.vcs_self.animate_info) < 2:
            gui_support.balloon.bind(
                self.min,
                "Enter the minimum value for the data set")
        else:
            gui_support.balloon.bind(
                self.min,
                "Enter the minimum value for each data set (e.g., 10, 20, 30, 40).")
        ...
        if len(animation.vcs_self.animate_info) < 2:
            gui_support.balloon.bind(
                self.max,
                "Enter the maximum value for the data set")
        else:
            gui_support.balloon.bind(
                self.max,
                "Enter the maximum value for each data set (e.g., 60, 70, 80, 90).")
    ...
    def mmexecute(self, eself, parent, result):
        ...
        if result == 'OK':
            ...
            if selection in ['Animation sets min and max',
                             'Animation sets min and max for all data sets']:
                eself.setting = 0
                eself.min = None
                eself.max = None
                ...
                if self.min.get() != '':
                    ...
                    while string.find(s, ',,', 0) != -1:
                        s = string.replace(s, ',,', ',')
                    if s[-1] == ',':
                        s = s[:-1]
                    eself.min = string.split(s, ',')
                    for i in range(len(eself.min)):
                        eself.min[i] = string.atof(eself.min[i])
                else:
                    eself.min = None
                ...
                if self.max.get() != '':
                    ...
                    for i in range(len(eself.max)):
                        eself.max[i] = string.atof(eself.max[i])
                else:
                    eself.max = None
...
def evt_select_anim_output_type(self, parent, type):
    if not self.animate_type.getvalue().lower() == 'ras':
        ...
        for b in self.downarrow_binds:
            self.downarrow_binds_funcs.append(
                self.cntrl_nxt_int.component('downarrow').bind(b))
        ...
        for b in self.uparrow_binds:
            self.uparrow_binds_funcs.append(
                self.cntrl_nxt_int.component('uparrow').bind(b))
        ...
        for b in self.zoom_comb_binds:
            self.zoom_comb_binds_funcs.append(
                self.zoom_comb.component('arrowbutton').bind(b))
        ...
        self.pan_hor_comb_binds = self.pan_hor_comb.component(
            'arrowbutton').bind()
        self.pan_hor_comb_binds_funcs = []
        for b in self.pan_hor_comb_binds:
            self.pan_hor_comb_binds_funcs.append(
                self.pan_hor_comb.component('arrowbutton').bind(b))
        ...
        self.pan_ver_comb_binds = self.pan_ver_comb.component(
            'arrowbutton').bind()
        self.pan_ver_comb_binds_funcs = []
        for b in self.pan_ver_comb_binds:
            self.pan_ver_comb_binds_funcs.append(
                self.pan_ver_comb.component('arrowbutton').bind(b))
        ...
        for i in range(n):
            ...
            self.cntrl_nxt_int.component('downarrow').bind(
                b,
                self.downarrow_binds_funcs[i])
        n = len(self.uparrow_binds)
        for i in range(n):
            b = self.uparrow_binds[i]
            self.cntrl_nxt_int.component('uparrow').bind(
                b,
                self.uparrow_binds_funcs[i])
        self.cntrl_nxt_int.configure(entry_state='normal')
        n = len(self.zoom_comb_binds)
        for i in range(n):
            self.zoom_comb.component("arrowbutton").bind(
                self.zoom_comb_binds[i],
                self.zoom_comb_binds_funcs[i])
        self.zoom_comb.component("entry").configure(state="normal")
        self.zoom_scl.configure(state="normal")
        n = len(self.pan_ver_comb_binds)
        for i in range(n):
            self.pan_ver_comb.component("arrowbutton").bind(
                self.pan_ver_comb_binds[i],
                self.pan_ver_comb_binds_funcs[i])
        ...
        for i in range(n):
            self.pan_hor_comb.component("arrowbutton").bind(
                self.pan_hor_comb_binds[i],
                self.pan_hor_comb_binds_funcs[i])
...
def create_animation_file_controls(self, parent, gui_parent, animation):
    ...
    self.animate_save = Tkinter.Canvas(
        frameb,
        bd=0,
    ...
    gui_support.balloon.bind(
        self.animate_save,
        "Save images to a file. Enter the file name by typing in the\nname in the entry window or by selecting the file icon to the left.")
    self.img_save = Tkinter.PhotoImage(
        file=os.path.join(
            cdms2.__path__[0],
            '..',
            '..',
            ...
            'bin',
            'animate_save.gif'))
    self.animate_save.create_image(
        0,
        0,
        anchor=Tkinter.NW,
        image=self.img_save)
    self.animate_save.bind(
        '<1>',
        A_Command(
            evt_enter_save_file,
            self,
    ...
    self.eny1.component('entry').bind(
        ...
        A_Command(
            ...
            animation,
            2))
    self.animate_type = Pmw.RadioSelect(
        frameb,
        buttontype='radiobutton',
        command=A_Command(
            evt_select_anim_output_type,
            self,
            parent))
    ...
    self.animate_load = Tkinter.Canvas(
        framec,
        bd=0,
        highlightthickness=0,
        width=32,
    ...
    gui_support.balloon.bind(
        self.animate_load,
        "Load images into memory. Enter the file name by typing in the\nname in the entry window or by selecting the file icon to the left.")
    self.img_load = Tkinter.PhotoImage(
        file=os.path.join(
            cdms2.__path__[0],
            '..',
            '..',
            ...
            'bin',
            'animate_load.gif'))
    self.animate_load.create_image(
        0,
        0,
        anchor=Tkinter.NW,
        image=self.img_load)
    self.animate_load.bind(
        '<1>',
        A_Command(
            evt_enter_load_file,
            self,
...
def evt_enter_save_file(self, parent, animation, who_called, event):
    ...
    if animation.vcs_self.animate.creating_animation_flg() == 1:
        return
    if animation.vcs_self.animate.run_animation_flg() == 1:
        return
    ...
    if string.find(sfile, '/') == -1:
        ...
        if os.access(directory, os.W_OK) == 1:
            ...
            showerror(
                ...
                "The specfied directory does not have write permission or does not exist. Please check the directory permissions.")
            self.eny1.setentry('')
            return
    else:
...
def evt_enter_load_file(
        self, parent, gui_parent, animation, who_called, event):
    # If creating or running an animation, then exit
    if animation.vcs_self.animate.creating_animation_flg() == 1:
        return
    if animation.vcs_self.animate.run_animation_flg() == 1:
    ...
    animation.vcs_self.animate.load_from_file(
        parent=gui_parent,
        load_file=lfile)
...
def create_animation_control_buttons(self, parent, gui_parent, animation):
    ...
    self.runanim_lbl.component('hull').configure(
        relief='sunken',
        borderwidth=2)
    self.runanim_btn = Tkinter.Button(self.runanim_lbl.interior(),
                                      ...
                                      background='cyan',
                                      command=A_Command(
        animation.vcs_self.animate.run)
    )
...
def evt_stop_animate(canvas):
    canvas.animate.stop()
    if canvas._canvas_gui is not None:
        canvas.animate.stop()
...
def create_control_images(self, parent, gui_parent, animation):
    ...
    if (save_file != ''):
        self.eny1.configure(entry_background='white')
        if save_file.split('.')[-1].lower() != self.animate_type.getvalue():
            save_file += '.' + self.animate_type.getvalue()
        try:
    ...
    animation.vcs_self.animate.create(
        parent=gui_parent,
        min=self.min,
        max=self.max,
        save_file=save_file,
...
def create_control_animation_frame(self, parent, animation):
    ...
    self.cntrl_slw_int = Pmw.Counter(frameg,
                                     labelpos='w',
                                     entryfield_modifiedcommand=A_Command(
                                         evt_cntrl_slw_cnt,
                                         self),
    ...
    gui_support.balloon.bind(
        self.cntrl_slw_int,
        'Pause the time (1/10 sec.) between each frame while animation is running.')
    ...
    gui_support.balloon.bind(
        self.cntrl_nxt_int,
        'Stop animation first, then step through the animation frames.')
...
def evt_cntrl_nxt_scl(self, animation, event):
    ...
    num_frames = animation.vcs_self.animate.number_of_frames()
...
def create_zoom_and_pan_animation_frame(self, parent, animation):
    ...
    self.zoom_comb = Pmw.ComboBox(framec,
                                  ...
                                  entry_width=7,
                                  entry_foreground='black',
                                  selectioncommand=A_Command(
                                      evt_zoom_image_cmb,
                                      self)
                                  )
    gui_support.balloon.bind(
        self.zoom_comb,
        'Zoom in on the animation image(s).')
    ...
    gui_support.balloon.bind(
        self.pan_hor_comb,
        'Pan the zoomed image(s) in the x direction..')
    ...
    gui_support.balloon.bind(
        self.pan_ver_comb,
        'Pan the zoomed image(s) in the y direction.')
