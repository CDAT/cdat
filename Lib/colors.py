from genutil.colors import rgb2str, str2rgb
...
def matplotlib2vcs(cmap,vcs_name=None):
  ...
  if vcs_name_final != vcs_name:
      warnings.warn("%s colormap name was already existing, your colormap name will be: %s" % (vcs_name, vcs_name_final))
  vcs_cmap = vcs.createcolormap(vcs_name_final)
  cmap_rgbs = cmap(range(0,cmap.N))
  for i in range(0,min(cmap.N,240)):
      vcs_cmap.setcolorcell(i,*([int(x/255.) for x in cmap_rgbs[i][:3]]))
