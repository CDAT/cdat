...
mkdir build_nightly ;\
cd build_nightly ;\
cmake -DCMAKE_INSTALL_PREFIX=${INSTALL_PATH} -DCDAT_BUILD_ESMF_ESMP=ON -DCDAT_BUILD_ESMF_PARALLEL=ON -DCDAT_BUILD_PARAVIEW=ON  -DCDAT_BUILD_VISIT=ON -DQT_QMAKE_EXECUTABLE=${QMAKE_EXE} ${XTRA_ARGS} .. ; \
cmake -DCMAKE_INSTALL_PREFIX=${INSTALL_PATH} -DCDAT_BUILD_ESMF_ESMP=ON -DCDAT_BUILD_ESMF_PARALLEL=ON -DCDAT_BUILD_PARAVIEW=ON  -DCDAT_BUILD_VISIT=ON -DQT_QMAKE_EXECUTABLE=${QMAKE_EXE} ${XTRA_ARGS} .. ; \
pwd ; \
make -j16 ; \
