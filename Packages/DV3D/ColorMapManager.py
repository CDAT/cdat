...
class AlphaManager():
    ...
    def setAlphaRange(self, arange):
        ...
        if self.n_col:
            self.ascale = (self.alpha_range[1] - self.alpha_range[0]) / (self.n_col - 1)
        self.graphEnabled = False
    ...
    def getAlphaValue(self, iCol):
        ...
        if self.graphEnabled:
            ...
            if iCol == 0:
                ...
                return gn0[1]
# print " getAlphaValue [%d/%d]: %f, ig = %d, ng = %d: %s " % ( iCol,
# self.n_col, dval, self.currentGraphNode, len(self.graph_data),
# str(self.graph_data) ); sys.stdout.flush()
