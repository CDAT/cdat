    ...
    def updateWidgetState(self):
        if self.numberOfImages > 1:
            if self.numberOfImages != self.numberOfStates:
                print>>sys.stderr, "Error, mismatch between numberOfImages(%d) and numberOfStates(%d) in Button %s " % (
                    self.numberOfImages, self.numberOfStates, self.id)
            else:
...
#                print "Button %s: widget state = %d " % ( self.id, self.buttonRepresentation.GetState() )
    ...
    def executeFunction(self, function):
        if function == Button.FuncToggleStateOn:
            self.setToggleState(1)
        elif function == Button.FuncToggleStateOff:
            self.setToggleState(0)
        ...
        if self.numberOfImages:
            ...
            for button_index in range(self.numberOfImages):
                ...
                if self.image_size is None:
                    self.image_size = image_data.GetDimensions()
    ...
    def createControlBar(self, name, interactor, build_args, **args):
        ...
        if cbar is None:
            if 'position' not in args:
                args['position'] = (0.55, 0.08)
            if 'orientation' not in args:
            ...
            for configFunct in bbar.configurableFunctions.values():
                if (configFunct.type == 'slider') and (configFunct.active) and (configFunct.group == self.DefaultGroup):
                    ...
                    if b.getState():
                        ...
                        if current_config_function is None:
                            current_config_function = configFunct
                        else:
                            if not configFunct.sameGroup(current_config_function):
                                print>>sys.stderr, "Error, interaction state conflict: %s vs %s " % (
                                    configFunct.name, bbar.InteractionState)
                                return
...
#        print " ButtonBar[%s]: %s" % ( name, str(self.position) )
    ...
    def updateWindowSize(self):
        self.windowSize = self.interactor.GetRenderWindow().GetSize() if (self.interactor is not None) else [100, 100]
    ...
    def placeButton(self, button, position, **args):
        ...
        if scale < 0.5:
            scale = 0.5
# print " ################################# Resize Button %s: ws=%d,
# scale=%s, pos=%s " % ( button.id, window_size, str(scale), str(position)
# )
...
#        print "Reposition: %d " % self.windowSize[0]
    ...
    def getScreenPosition(self, normalized_display_position, buffered=True, **args):
        ...
        screen_pos = self.vtk_coord.GetComputedDisplayValue(ren) if (ren is not None) else [100, 100]
        ...
        if self.orientation == Orientation.Vertical:
            ...
            position_offset[1] = 0
        if buffered:
            screen_pos = self.getBufferedPos(screen_pos, position_offset)
# print " GetScreenPosition [",  self.name, "], position = ", str(
# normalized_display_position ), "], screen position = ", str( screen_pos
# )
        return screen_pos
    ...
    def getBufferedPos(self, screen_pos, position_offset=[0, 0]):
        ...
        for ic in range(2):
            if self.origin[ic]:
                buff_screen_pos[ic] = screen_pos[ic] - self.buffer[ic] - position_offset[ic]
            else:
                buff_screen_pos[ic] = screen_pos[ic] + self.buffer[ic] + position_offset[ic]
    ...
    def clear(self, **args):
        ...
        for b in self.buttons:
            if (current_button_id is None) or (b.id != current_button_id):
                if b.getState() != 0:
                    b.setButtonState(0)
...
# print " ---> change Button Activation[%s], activate = %s, state = %s" %
# ( button_name, str(activate), str(state) )
        if button:
            if activate:
                button.activate()
            else:
    ...
    def reset(self, active_state=None):
        if (active_state is None) or (self.name != active_state):
            self.hide()
...
class ButtonBarWidget(ButtonBar):
    ...
    def __init__(self, handler, name, interactor, **args):
        ...
        self.slider_postions = [[[0.25, 0.75]], [[0.01, 0.48], [0.52, 0.99]], [[0.01, 0.3], [
            0.35, 0.7], [0.75, 0.99]], [[0.01, 0.24], [0.26, 0.49], [0.51, 0.74], [0.76, 0.99]]]
    ...
    def setVisibility(self, isVisible):
        ...
        for (process_mode, interaction_state, swidget) in self.currentControls.values():
            if not isVisible:
                swidget.GetRepresentation().VisibilityOff()
            else:
                swidget.GetRepresentation().VisibilityOn()
        ...
        if (b.getState() != state) or (not b.toggle) or force:
            ...
            if state > 0:
                ...
                if not b.toggle:
                    ...
                    if self.name == "Plot":
                        self.resetInteractionButtons(b, 0)
                    else:
                        self.InteractionState = None
                    ...
                    for pindex in positions:
                        self.releaseSlider(pindex)
    ...
    def updateSliderWidgets(self, values):
        for index, value in enumerate(values):
            if value is not None:
                self.setSliderValue(index, value)
        self.render()
    ...
    def initializeSliderPosition(self, index):
        try:
            ...
            if swidget:
                ...
                value = ((srep.GetMinimumValue() + srep.GetMaximumValue()) / 2.0) if (values is None) else values[0]
                srep.SetValue(value)
...
#        eventTranslator.SetTranslation (vtk.vtkCommand.RightButtonPressEvent, vtk.vtkWidgetEvent.ModifyEvent )
    ...
    def commandeerControl(self, index, label, bounds, tvals):
        if bounds is None:
            return
# print " CommandeerSlider[%d]: ('%s') %s: %s in %s " % ( index, label,
# self.InteractionState, str(tvals), str(bounds) )
        widget_item = self.currentControls.get(index, None)
        isButtonWidget = isinstance(label, list)
        if widget_item is None:
            if isButtonWidget:
...
#            print "Releasing slider[%d]: %s " % ( index, interaction_state )
    ...
    def getSliderEnabled(self, index):
        (process_mode, interaction_state, swidget) = self.currentControls.get(index, (None, None, None))
        if swidget:
            return swidget.GetEnabled()
        return False

    def clearInteractions(self):
        if self.InteractionState is not None:
    ...
    def processInteractionEvent(self, obj=None, event=[]):
        if (self.InteractionState is not None):
            ...
            if config_function is not None:
                config_function.processInteractionEvent(
                    ["UpdateConfig", self.getSliderIndex(obj), srep, event, self.interaction_priority])
                self.interaction_priority = 0
            else:
                print>>sys.stderr, " FAILED processInteractionEvent[%s]: ( %s %d )" % (
                    self.name, self.InteractionState, self.process_mode)
...
#        print " processStartInteractionEvent: ( %s %d %d )" % ( self.InteractionState, self.process_mode, shift_key )
        if (self.InteractionState is not None):
            ...
            if config_function is not None:
                ...
                if shift_key:
                    self.processWidgetShiftCommand(obj, slider_index, config_function)
                config_function.processInteractionEvent(["StartConfig", slider_index, self.interaction_priority])
            else:
                print>>sys.stderr, " FAILED processStartInteractionEvent[%s]: ( %s %d )" % (
                    self.name, self.InteractionState, self.process_mode)

    def processWidgetShiftCommand(self, obj, slider_index, config_function):
        pass
# print " --------->>>>> processWidgetToggleCommand:  %s %d %s" % (
# str(obj), slider_index, str(config_function)  )
    ...
    def processEndInteractionEvent(self, obj, event):
        ...
        if (self.InteractionState is not None):
            ...
            if config_function is not None:
                ...
                print>>sys.stderr, " FAILED processEndInteractionEvent[%s]: ( %s %d )" % (
                    self.name, self.InteractionState, self.process_mode)

    def startConfiguration(self, x, y, config_types):
        if (self.InteractionState is not None) and not self.configuring:
            configFunct = self.configurableFunctions[self.InteractionState]
...
#                if (configFunct.type == 'leveling'): self.getLabelActor().VisibilityOn()
    ...
    def getInteractionState(self, key):
        for configFunct in self.configurableFunctions.values():
            if configFunct.matches(key):
                return (configFunct.name, configFunct.persisted, self)
...
#                        self.setSliderValue( position_index, slicePosition.getValue() )
                    ...
                    if self.isSliderVisible(position_index) or force_enable:
                        ...
                        self.commandeerControl(position_index, configFunct.sliderLabels[
                                               0], configFunct.getSliderBounds(), tval)
                    ...
                    for slider_index in range(4):
                        self.setSliderVisibility(slider_index, slider_index < n_active_sliders)
                    for slider_index in range(4):
                        if self.isSliderVisible(slider_index) and (len(tvals) > slider_index):
                            self.commandeerControl(
                                slider_index,
                                configFunct.sliderLabels[slider_index],
                                configFunct.getSliderBounds(),
                                tvals[slider_index])
    ...
    def updateChildState(self, child_button, parent_name):
        if len(child_button.parents):
            if (parent_name in child_button.parents):
                ...
                if (parent_button is not None) and parent_button.getState():
                    if self.visible:
...
#                    print "Dectivate child: ", child_button.id
    ...
    def initConfigState(self, **args):
        ...
        child_activations = []
        for button in self.buttons:
            if (active_button is None) and (button.getState() > 0):
