...
#        memoryLogger.log("finished clearing image cache")
    ...
    def setupTimeAxis(self, var, **args):
        ...
        if self.timeAxis:
            ...
            try:
                if self.referenceTimeUnits is None:
...
#             self.timeIndex = 0
#             self.useTimeIndex = False
#            print "Set Time [mid = %d]: %s, NTS: %d, Range: %s, Index: %d
#                   (use: %s)" % ( self.moduleID, str(self.timeValue),
#                   self.nTimesteps, str(self.timeRange), self.timeIndex,
#                   str(self.useTimeIndex) )
...
#            if self.newDataset: ModuleStore.archiveCdmsDataset( dsetId, self.cdmsDataset )
        ...
        if hasattr(cdms_var, 'url'):
            self.generateOutput(roi=intersectedRoi, url=cdms_var.url)
        else:
...
#                print "Set Time: %s, NTS: %d, Range: %s, Index: %d (use: %s)" % (
#                    str(self.timeValue), self.nTimesteps, str(self.timeRange),
#                     self.timeIndex, str(self.useTimeIndex) )
#                print "Time Step Labels: %s" % str( self.timeLabels )

    def setTimestep(self, iTStep, restart=False):
        displayText = None
        if (self.timeRange is None) or self.timeRange[0] == self.timeRange[1]:
            self.running = False
        else:
...
#                     print " ** Update Animation, timestep = {0}, index = {1},
#                            timeValue = %.3f, useIndex = %d, timeRange = %s "
#                            % ( self.iTimeStep, timeIndex, relTimeValueRefAdj,
#                            self.uniformTimeRange, str( self.timeRange ) )
...
# ############################################" % iTS
    ...
    def getOutputRecord(self, ndim=-1):
        ...
        if portData:
            ...
            if not orecs:
                raise Exception('No Variable selected for dataset %s.' % self.datasetId)
            for orec in orecs:
                if (ndim < 0) or (orec.ndim == ndim):
                    return orec
        return None
...
#         return 0 if dt <= 0.0 else int( round( ( self.timeValue.value - self.timeRange[2] ) / dt ) )
    ...
    def getFileMetadata(self, orec, **args):
        ...
        if fieldData:
            ...
            for ia in range(na):
                ...
                if (aname is not None) and aname.startswith('metadata'):
        ...
        for varRec in varList:
            ...
            if ds:
                ...
                self.setupTimeAxis(var, **args)
            selectedLevel = orec.getSelectedLevel() if (self.currentLevel is None) else self.currentLevel
            ...
            if lat_axis is not None:
                vmd['lat'] = lat_axis.getValue()
            lon_axis = var.getLongitude()
            if lon_axis is not None:
                vmd['lon'] = lon_axis.getValue()
    ...
    def levOrderingDownward(self, lev_axis):
        ...
        if lev_axis.attributes.get('positive', '') == 'down' and ascending_values:
            return True
        elif lev_axis.attributes.get('positive', '') == 'up' and not ascending_values:
            return True
        return False
    ...
    def getImageData(self, orec, **args):
        """
        This method converts cdat data into vtkImageData objects.
        The ds object is a CDMSDataset instance which wraps a CDAT CDMS
        Dataset object.

        The ds.getVarDataCube method execution extracts a CDMS variable object
        (varName) and then cuts out a data slice with the correct axis ordering
        (returning a NumPy masked array).
        The array is then rescaled, converted to a 1D unsigned short array,
        and then wrapped as a vtkUnsignedShortArray using the
        vtkdata.SetVoidArray method call.

        The vtk data array is then attached as point data to a vtkImageData
        object, which is returned.
        The CDAT metadata is serialized, wrapped as a vtkStringArray,
        and then attached as field data to the vtkImageData object.
        """
        varList = orec.varList
        npts = -1
        if len(varList) == 0:
...
#        print " Get Image Data: varList = %s " % str( varList )
        for varRec in varList:
            ...
            if len(varNameComponents) == 1:
                ...
                varName = varNameComponents[1]
            ds = self.cdmsDataset[dsid]
            if (ds is None) and (self.df is not None):
                ds = self.df
            if ds is not None:
                var = ds.getVariable(varName)
                if var is not None:
            ...
            cell_coords = (0, 0)
            roiStr = ":".join([("%.1f" % self.cdmsDataset.gridBounds[i])
                               for i in range(4)]) if self.cdmsDataset.gridBounds else ""
            ...
            if varDataSpecs is None:
                if varName == '__zeros__':
                    ...
                    tval = None if (self.outputType == CDMSDataType.Hoffmuller) else [
                        self.timeValue, iTimestep, self.useTimeIndex]
                    varDataMasked = self.cdmsDataset.getVarDataCube(
                        dsid, varName, tval, selectedLevel, cell=cell_coords)
                    if varDataMasked.id != 'NULL':
                        varDataSpecs = self.getGridSpecs(
                            varDataMasked,
                            self.cdmsDataset.gridBounds,
                            self.cdmsDataset.zscale,
                            self.outputType,
                            ds)
                        if (exampleVarDataSpecs is None) and (varDataSpecs is not None):
                            exampleVarDataSpecs = varDataSpecs
                        range_min = varDataMasked.min()
                        if type(range_min).__name__ == "MaskedConstant":
                            range_min = 0.0
                        range_max = varDataMasked.max()
                        if type(range_max).__name__ == 'MaskedConstant':
                            range_max = 0.0
                        ...
                        if (scalar_dtype == np.float32) or (scalar_dtype == np.float64):
                            ...
                            shift = -range_min
                            scale = (self._max_scalar_value) / (range_max -
                                                                range_min) if (range_max > range_min) else 1.0
                        ...
                        if npts == -1:
                            ...
                            assert(npts == array_size)

                        var_md['range'] = (range_min, range_max)
                        var_md['scale'] = (shift, scale)
                        varDataSpecs['newDataArray'] = varData
# print " ** Allocated data array for %s, size = %.2f MB " % ( varDataId,
# (varData.nbytes /(1024.0*1024.0) ) )
                        md = varDataSpecs['md']
                        ...
                        if self.timeLabels is not None:
                            md['base_time'] = [str(ct) for ct in self.timeLabels]
                        tvar = var if var is not None else self.vars[0]
                        ...
                        axis = tvar.getTime()
                        if axis is not None:
                            md['time'] = [str(tc) for tc in axis.asComponentTime()]
                        else:
                            md['time'] = md['base_time']
...
#            print " ********************* Create Image Data, extent = %s,
#                   spacing = %s ********************* " % (
#                   str(extent), str(gridSpacing) )
...
#                    print "Add array to PointData: %s " % ( varName  )
                    if (scalars is None) and (varName != '__zeros__'):
                        scalars = varName
...
#             print "Add array %d to PointData: %s (%s)" % (
#                   iArray, pointData.GetArrayName(iArray), scalars.GetName()  )
        try:
            if (self.outputType == CDMSDataType.Vector):
                vtkdata = getNewVtkDataArray(scalar_dtype)
            ...
            if len(vars) == 0:
                raise Exception('No dataset variables selected for output %s.' % orec.name)
            for varDataId in varDataIds:
    ...
    def getAxisValues(self, axis, roi):
        ...
        if roi:
            if axis.isLongitude():
                ...
                bounds = [roi[1], roi[3]] if (roi[3] > roi[1]) else [roi[3], roi[1]]
        if bounds:
            if len(values) < 2:
                values = bounds
            else:
        ...
        try:
            ...
            for iCoord in range(2):
                ...
                if (len(axisvals.shape) > 1):
                    return current_roi
                # max( current_roi[iCoord], roiBounds[0] ) if current_roi else roiBounds[0]
                newRoi[iCoord] = axisvals[0]
                # min( current_roi[2+iCoord], roiBounds[1] ) if current_roi else roiBounds[1]
                newRoi[2 + iCoord] = axisvals[-1]
            if (current_roi_size == 0):
                return newRoi
...
#                    gridSpacing[ iCoord ] = zscale
        ...
        if gridBounds[2] > gridBounds[3]:
            ...
            gridBounds[3] = tmp
        gridSpecs = {}
        # , 'lat':self.lat, 'lon':self.lon, 'lev':self.lev, 'time': self.timeAxis }
        md = {'datasetId': self.datasetId, 'bounds': gridBounds}
        ...
        if dset:
            try:
                gridSpecs['attributes'] = dset.dataset.attributes
            except:
                gridSpecs['attributes'] = dset.attributes
        return gridSpecs
