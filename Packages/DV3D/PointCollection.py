...
# print " GetDataBlock, var.shape = %s, grid = %s, ts = %d, newshape = %s
# " % ( str(var.shape), str((self.istart,self.istep)), self.iTimeStep,
# str(np_var_data_block.shape) )
            ...
            if height_varname and (height_varname != self.hgt_var) and (height_varname != 'Levels'):
                ...
                if hgt_var:
                    ...
                    if self.missing_value:
                        np_hgt_var_data_block = numpy.ma.masked_equal(np_hgt_var_data_block, self.missing_value, False)
                    zdata = np_hgt_var_data_block.astype(numpy.float32)
                    print " setPointHeights: zdata shape = %s " % str(zdata.shape)
                    sys.stdout.flush()
...
#                 return  self.processCoordinates( lat, lon )
    ...
    def stepTime(self, **args):
        ...
        if process:
            ...
            if id(var_data) == id(None):
                ...
                if self.missing_value:
                    var_data = numpy.ma.masked_equal(np_var_data_block, self.missing_value, False)
                else:
    ...
    def extractMetadata(self):
        ...
        if axis:
            self.metadata['lon'] = axis.getValue()
        axis = self.var.getLatitude()
        if axis:
            self.metadata['lat'] = axis.getValue()
        axis = self.var.getLevel()
        if axis:
            self.metadata['lev'] = axis.getValue()
...
#            print " computeThresholdRange: %f %f " % ( vmin, vmax )
            if not isNone(var_data):
                threshold_mask = numpy.logical_and(
                    numpy.greater_equal(
                        var_data, vmin), numpy.less_equal(
                        var_data, vmax))
