...
#         self.cachedFileVariables = {}
    ...
    def getLevAxis(self):
        for axis in self.dataset.axes.values():
            if axis.isLevel() or PlotType.isLevelAxis(axis):
                return axis
        return None
    ...
    def getLevBounds(self, levaxis):
        ...
        if levaxis:
            ...
            if levaxis:
                if levaxis.attributes.get('positive', '') == 'down' and ascending_values:
                    levbounds = slice(None, None, -1)
                elif levaxis.attributes.get('positive', '') == 'up' and not ascending_values:
                    levbounds = slice(None, None, -1)
        return levbounds
...
#        print "Reading Variable %s, attributes: %s" % ( varName, str(varData.attributes) )
        ...
        decimationFactor = 1
        if decimation:
            decimationFactor = decimation[0] + 1
#        try:
        ...
        if decimationFactor == 1:
            ...
            if varGrid:
                ...
                latBounds = [gridBounds[3], gridBounds[1]] if latVals[
                    0] > latVals[1] else [gridBounds[1], gridBounds[3]]
...
#                                              values=[ nodata_value ] ) if nodata_value else None
            ...
            vc = cdutil.VariableConditioner(
                source=self.cdmsFile,
                var=varName,
                cdmsKeywords=args1,
...
#            print>>sys.stderr, ' Exception getting var slice: %s ' % str( err )
    ...
    def getFileVarDataCube(self, varName, decimation, **args):
        ...
        gridMaker = None
        decimationFactor = 1
        order = 'xyt' if (timeBounds is None) else 'xyz'
        if decimation:
            decimationFactor = decimation[0] + 1
        try:
            nts = self.dataset['time'].shape[0]
            if (timeIndex is not None) and useTimeIndex:
        ...
        if lonBounds is not None:
            if (lonBounds[1] - lonBounds[0]) < 355.0:
                if lonBounds[0] < LonMin:
                    lonBounds[0] = LonMin
                if lonBounds[1] > LonMax:
                    lonBounds[1] = LonMax
...
#        self.cachedTransVariables = {}
    ...
    def getTimeValues(self, asComp=True):
        ...
        try:
            ...
            for iTime in range(self.timeRange[0], self.timeRange[1] + 1):
                ...
                if asComp:
                    time_values.append(tval.tocomp())
                else:
    ...
    def setReferenceVariable(self, selected_grid_id):
        try:
            if (selected_grid_id is None) or (selected_grid_id == 'None'):
                return
    ...
    def getReferenceDsetId(self):
        if self.referenceVariable is None:
            return self.datasetRecs.keys()[0]
...
#            del tvar
    ...
    def getVarDataTimeSlice(self, dsid, varName, timeValue):
        ...
        if dsid:
            dsetRec = self.datasetRecs[dsid]
            if varName in dsetRec.dataset.variables:
                rv = dsetRec.getVarDataTimeSlice(
                    varName,
                    timeValue,
                    self.gridBounds,
                    self.decimation,
...
#                return vc.get( returnTuple=0 )
            ...
            if dsetRec:
                if varName in dsetRec.dataset.variables:
                    args = {
                        'time': timeValues,
                        'lev': levelValues,
                        'refVar': self.referenceVariable,
                        'refLev': self.referenceLev}
    ...
    def getTransVarDataCube(self, varName, transVar, decimation, **args):
        ...
        if levaxis:
            values = levaxis.getValue()
            ascending_values = (values[-1] > values[0])
            invert_z = ((levaxis.attributes.get('positive', '') == 'down') and ascending_values) or (
                (levaxis.attributes.get('positive', '') == 'up') and not ascending_values)
...
#        print "Reading Variable %s, attributes: %s" % ( varName, str(transVar.attributes) )
        ...
        if (decimationFactor > 1) or lonBounds or latBounds:
            ...
            if latBounds:
                if (latVals[-1] > latVals[0]):
                    latRange = [latBounds[0], latBounds[-1]
                                ] if (latBounds[-1] > latBounds[0]) else [latBounds[-1], latBounds[0]]
                else:
                    latRange = [latBounds[0], latBounds[-1]
                                ] if (latBounds[-1] < latBounds[0]) else [latBounds[-1], latBounds[0]]
                    invert_y = True
            ...
            if varLatInt:
                if invert_y:
                    args1['lat'] = slice(varLatInt[1], varLatInt[0], -decimationFactor)
                else:
                    args1['lat'] = slice(varLatInt[0], varLatInt[1], decimationFactor)
        ...
        if levaxis:
            if level:
                args1['lev'] = float(level)
            elif invert_z:
                args1['lev'] = slice(None, None, -1)
...
# varName, str(rv.shape), str(transVar.shape), str(args1) )
    ...
    def getVarDataTimeSlices(self, varList, timeValue):
        ...
        for (dsid, varName) in varList:
            ...
            if not vc0:
                vc0 = cdutil.VariableConditioner(varTimeSlice)
            else:
                timeSlices.append(varTimeSlice)
        for varTimeSlice in timeSlices:
            ...
            if not condTimeSlices:
                condTimeSlices.append(condTimeSlice0)
    ...
    def addDatasetRecord(self, dsetId, relFilePath):
        ...
        if (cdmsDSet is not None) and (cdmsDSet.cdmsFile == relFilePath):
            return cdmsDSet
        try:
    ...
    def getVariableList(self, ndims):
        ...
        for dsetRec in self.datasetRecs.values():
            for var in dsetRec.dataset.variables:
                ...
                if var_ndim == ndims:
                    vars.append('%s*%s' % (dsetRec.id, var))
        return vars
...
class SerializedInterfaceSpecs:
    ...
    def parseInputSpecs(self, serializedConfiguration):
        ...
        for iCell in range(len(cellInputSpecs)):
            ...
            if len(cellMetadata) > 1:
                self.cells.append((cellMetadata[0], cellMetadata[1]))
...
# print " TimeRange Specs: ", str( values ), str( start_time ), str(
# end_time ), str( self.referenceTimeUnits )
        ...
        for dataset in dataset_list:
            ...
            if time_values:
                ...
                if tbounds[0] > start_time:
                    start_time = tbounds[0]
                if dt == 0.0:
                    end_time = start_time
                elif tbounds[1] < end_time:
                    end_time = tbounds[1]
                if dt < min_dt:
                    min_dt = dt
        for dataset in dataset_list:
        ...
        if min_dt == float('inf'):
            ...
            nTS = int(((end_time - start_time) / min_dt) + 0.0001)
...
#                print " ________________________________________________________ "
                ...
                for iVar in range(len(self.varSpecs)):
                    ...
                    if iVar == 0:
                        self.ref_var = varSpec
                    self.datasetModule.setVariableRecord("VariableName%d" % iVar, varSpec)
            else:
                time_range = self.getInputValue("timeRange")
                self.timeRange = [int(time_range[0]), int(time_range[1]), float(
                    time_range[2]), float(time_range[3])] if time_range else None
    ...
    #                print " - addDatasetRecord: ", str( datasetId ), str( cdmsFile )
            ...
            if inputSpecs:
                self.persistDatasetParameters()
            self.setResult('dataset', self.datasetModule)
# print " ......  Start Workflow, dsid=%s, zscale = %.2f ......  " % (
# self.datasetModule.getDsetId(), zscale )
