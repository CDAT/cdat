...
class ExecutionDataPacket:
    ...
    def __init__(self, msg_type, node_index, data_object):
        ...
        self.metadata = {}
# print "ExecutionDataPacket[%d]: type=%s, shape=%s" % ( node_index,
# self.getTypeStr(msg_type), str(data_object.shape) )
...
class PointCollectionExecutionTarget:
    ...
    def execute(self, args):
        try:
            ...
            # print "execute PointCollectionExecutionTarget(%s): init_args=[%s],
            # result-> queue " % ( str(args), str(self.init_args) );
            # sys.stdout.flush()
...
#        self.threshold_target = "vardata"
    ...
    def printLogMessage(self, msg_str, **args):
        ...
        if error:
            print>>sys.stderr, " Proxy Node %d Error: %s" % (self.pcIndex, msg_str)
        else:
...
# Scalar Range: %s " % str( self.current_scalar_range )
        self.mapper.Modified()
        self.actor.Modified()
        self.actor.SetVisibility(True)
...
#                pt0 = self.vtk_spherical_points.GetPoint(0)
    # print "VTK Set point Heights, samples: %s %s %s " % ( str( ptheights[0]
    # ), str( self.np_sp_grid_data[0] ), str( pt0 ) )
...
#            if self.actor.GetVisibility():
    ...
    def getBounds(self, **args):
        ...
        if topo == PlotType.Spherical:
            return [-self.earth_radius, self.earth_radius, -self.earth_radius,
                    self.earth_radius, -self.earth_radius, self.earth_radius]
        else:
...
class vtkSubProcPointCloud(vtkPointCloud):
    ...
    def runProcess(self, procType, **args):
        if procType == PCProc.Subset:
            self.generateSubset(**args)
        elif procType == PCProc.ZScaling:
            self.generateZScaling(**args)
        elif procType == PCProc.Timestep:
            self.stepTime(**args)
...
# print " vtkSubProcPointCloud[%d]: current_subset_specs: %s (%s) " % (
# self.pcIndex, self.current_subset_specs, str(args) ); sys.stdout.flush()
...
#            self.printLogMessage( " vtkSubProcPointCloud --->> Generate subset: %s " % str(self.current_subset_specs) )
    ...
    def generateZScaling(self, **args):
        ...
        if cached_zscale_value != zscale_value:
            ...
            # print " Generate Z Scaling [P-%d]: %s, zscale_value = %s, op_specs =
            # (%s)" % ( self.pcIndex, str( args ), str( zscale_value ), str(op_specs)
            # )
            self.arg_queue.put(op_specs, False)
...
#                 self.printLogMessage(  " vtkSubProcPointCloud --->> Process Results, Args: %s " % str(result['args']) )
            self.updateVertices()
            # print " processResults[ %d ] : INDICES, metadata = %s " % (
            # self.pcIndex, str(result.metadata)); sys.stdout.flush()
...
#        print "generateZScaling: op_specs = %s " % str( op_specs )
    ...
    def generateSubset(self, **args):
        # print " ++++++++++++++++++++++ vtkLocalPointCloud[%d].generateSubset:
        # current_subset_specs: %s (%s) " % ( self.pcIndex,
        # self.current_subset_specs, str(args) )
        self.current_subset_specs = args.get('spec', self.current_subset_specs)
...
#            print "Set crange: ", str( self.crange )
    ...
    def initialize(self, init_args=None, **args):
        if init_args:
            ...
            self.point_collection.initPoints()
        self.np_points_data = self.point_collection.getPoints()
        self.vrange = self.point_collection.getVarDataRange()
        self.initPoints(**args)
        self.createPolydata(**args)
...
#        print "initialize: Set grid bounds: %s " % str( self.grid_bounds )
    ...
    def getCenter(self):
        return (
            self.point_collection.xcenter,
            self.point_collection.ycenter,
            self.point_collection.xwidth,
            self.point_collection.ywidth)
...
# print "
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# startCheckingProcQueues, interactor = ",
# self.interactor.__class__.__name__
...
# print " --> updateNumActiveCollections(%.2f): %d, pc_base_index: %d " %
# ( res, n_collections, pc_base_index ); sys.stdout.flush()
    ...
    def runProcess(self, procType, **args):
        ...
        if self.nActiveCollections > pc_base_index:
            ...
            for pc_item in self.point_clouds.items():
                run_process = allow_processing and (
                    pc_item[0] < self.nActiveCollections) and (
                    pc_item[0] >= pc_base_index)
