...
def saveAnimation(saveDir, animation_frames):
    ...
    print "Saving recorded animation to %s" % movie
    sys.stdout.flush()
...
def saveAnimationFFMpeg(animation_frames, saveDir):
    ...
    files = []
    print " Saving animation (%d frames) to '%s'" % (len(animation_frames), saveDir)
    sys.stdout.flush()
    for index, frame in enumerate(animation_frames):
...
class DV3DPlot():
    ...
    def __init__(self, **args):
        ...
        self.plotConstituents = {
            'Slice': 'SliceRoundRobin',
            'Volume': 'ToggleVolumePlot',
            'Surface': 'ToggleSurfacePlot'}
...
#        self.buttons = {}
        ...
        interactionButtons.addSliderButton(
            names=['BasemapOpacity'],
            key='B',
            toggle=True,
            label='Basemap Opacity',
            sliderLabels='Basemap Opacity',
            interactionHandler=self.processBasemapOpacityCommand,
            range_bounds=[
                0.0,
                1.0],
            initValue=0.5)
        ...
        interactionButtons.addSliderButton(
            ...
            toggle=True,
            label='Animation',
            sliderLabels='Speed (Step Delay)',
            interactionHandler=self.processAnimationCommand,
            range_bounds=[
                0.0,
                2.0],
            initValue=0.0)
    ...
    def getControlBar(self, config_function, build_args, **args):
        control_bar = self.buttonBarHandler.createControlBar(
            config_function.cfg_state,
...
#        print "processTimerEvent: %d %d " % ( eid, etype )
    ...
    def captureFrame(self, **args):
        ...
        if ignore_alpha:
            frameCaptureFilter.SetInputBufferTypeToRGB()
        else:
            frameCaptureFilter.SetInputBufferTypeToRGBA()
...
#    plot = self.canvas.backend.plotApps[ self.gm ]
    ...
    def createCTestFunction(self, **args):
        try:
            ...
            if roi:
                mdataStrs.append(' roi=%s' % str(roi))
            ...
            if template is not None:
                mdataStrs.append(' template="%s"' % template)
...
#            print " processCameraCommand: "
    ...
    def processShowColorbarCommand(self, args, config_function=None):
        if args and args[0] == "InitConfig":
            constituent = 'Slice'
            state = args[1]
            cs_button = self.getConstituentSelectionButton(
                config_function, [
                    (self.plotConstituents.keys(), ), self.processColorbarConstituentSelection], (0.9, 0.2))
            if state:
                cs_button.show()
            else:
                cs_button.hide()
    ...
    def processChooseColormapCommand(self, args, config_function):
        ...
        if args and args[0] == "StartConfig":
            ...
            state = args[1]
            if (self.colormapWidget is None):  # or self.colormapWidget.checkWindowSizeChange():
            ...
            if len(args) == 1:
                ...
                self.colormapWidget.toggleVisibility(state=args[1])
            ...
            if self.colormapWidget is not None:
    ...
    def isConstituentConfigEnabled(self, constituent):
        ...
        return param.getValue('ConfigEnabled', True) if (param is not None) else True
    ...
    def processAnimationCommand(self, args, config_function=None):
        ...
        if args and args[0] == "StartConfig":
            ...
            state = args[1]
            bbar = self.getControlBar(
                config_function, [
                    ("Step", "Run", "Stop", ("Record", True)), self.processAnimationStateChange], mag=1.4)
            if state == 1:
                self.updateTextDisplay(config_function.label)
                bbar.show()
                if self.animating:
...
# print " setInteractionState -- Key Press: %c ( %d: %s ), event = %s " %
# ( key, ord(key), str(keysym), str( event ) )
        alt = (keysym is not None) and keysym.startswith("Alt")
        if alt:
            self.isAltMode = True
        else:
...
#                     self.render()
    ...
    def createRenderer(self, **args):
        ...
        if self.renderWindowInitSize is not None:
...
#            RenderWindow.AddObserver( 'AnyEvent', self.onAnyWindowEvent )
    ...
    def buildPlotButtons(self, **args):
        ...
        if ispec is not None:
            ...
            lev = md.get('lev', None)
            if (lev is None) and (plotType == 'xyz'):
        ...
        if (self.type == '3d_vector') or not enable_3d_plots:
            ...
            b = bbar.addSliderButton(
                names=['ZSlider'],
                key='z',
                visible=enable_3d_plots,
                toggle=True,
            ...
            if vs_button is not None:
                vs_button.setVisibility(False)
        else:
...
#            print " init ToggleVolumePlot: state=%s, cfg=(%s) " % ( str(state), str(config_function.initial_value) )
            ...
            if state_val:
                self.toggleVolumeVisibility(state_val)
            self.parameter_initializing = False
        elif args and args[0] == "InitConfig":
            state = args[1]
    ...
    def processConfigurationToggle(self, args, config_function=None):
        if args and args[0] == "Init":
            ...
            state = config_function.getState()
            if state:
                self.toggleCongurationButtons(state)
    ...
    def getInteractionButtons(self):
        ...
        if bbar is None:
            bbar = self.addInteractionButtons()
        return bbar
    ...
    def onWindowRenderEvent(self, caller=None, event=None):
        renwin = self.renderWindow if (caller is None) else caller
...
#                 self.onRenderWindowResize()
    ...
    def getColormapManager(self, constituent, **args):
        ...
        if (invert is not None):
            cmap_mgr.invertColormap = invert
        if (smooth is not None):
            cmap_mgr.smoothColormap = smooth
        if name:
            cmap_mgr.load_lut(name)
        return cmap_mgr
    ...
    #        self.updateStereo( enableStereo )
            colormapManager = self.getColormapManager(
                ...
                index=cmap_index,
                units=var_units)
            if(colormapManager.colorBarActor is None):
...
#         self.renderWindowInteractor.Start()
    ...
    def initCamera(self, d=None, center=None):
        # print " -------------------------- >>>>> ---------------------------
        # >>>>  initCamera:  ", str( ( self.xcenter, self.ycenter, d ) )
        cameraParameter = self.cfgManager.getParameter('Camera')
        cPosition = cameraParameter.getValue('Position', None)
        if cPosition is None:
            if self.renderWindowSize is not None:
