...
class MapManager:
    ...
    def __init__(self, **args):
        ...
        if self.map_cut_size[0] > 360.0:
            self.map_cut_size[0] = 360.0
        if self.map_cut_size[1] > 180.0:
            self.map_cut_size[1] = 180.0
...
#        print "Set Corner pos: %s, roi: %s " % ( str(self.x0), str(self.roi) )
    ...
    def RollMap(self, baseImage):
        if self.world_cut == self.map_cut:
            return baseImage
...
# print "  %%%%%% Roll Map %%%%%%: world_cut=%.1f, map_cut=%.1f,
# newCut=%.1f " % ( float(self.world_cut), float(self.map_cut),
# float(newCut) )
        imageLen = x1 - x0 + 1
    ...
    def getBoundedMap(self, baseImage, dataLocation):
        ...
        overlapsBorder = (
            self.NormalizeMapLon(
                dataLocation[0] -
                selectionDim[0]) > self.NormalizeMapLon(
                dataLocation[0] +
                selectionDim[0]))
        if overlapsBorder:
