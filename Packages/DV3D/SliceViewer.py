...
#         self.addUVCDATConfigGuiFunction( 'contourColormap', ColormapConfigurationDialog, 'K', label='Choose Contour Colormap', setValue=lambda data: self.setColormap(data,1) , getValue=lambda: self.getColormap(1), layerDependent=True, isValid=self.hasContours, group=ConfigGroup.Color )
        ...
        self.addConfigurableSliderFunction(
            'xSlider', 'x', sliderLabels='X Slice Position', label="Slicing", position=[
                0, 3], interactionHandler=self.processSlicingCommand)
        self.addConfigurableSliderFunction(
            'ySlider', 'y', sliderLabels='Y Slice Position', label="Slicing", position=[
                1, 3], interactionHandler=self.processSlicingCommand)
        self.addConfigurableSliderFunction(
            'zSlider', 'z', sliderLabels='Z Slice Position', label="Slicing", position=[
                2, 3], interactionHandler=self.processSlicingCommand)
...
#                self.observerTargets.add( self.planeWidgetY )
        ...
        if (contour_ispec is not None) and (contour_ispec.input() is not None) and (self.contours is None):
...
#                    print " >>>>> Current Image Value: %d,  pos = %s " % ( image_value, str(cpos) ) # , str(wpos) )
            ...
            if action == ImagePlaneWidget.Pushing:
                ...
                if iAxis == 0:
                    p1 = caller.GetPoint1()
# print " >++++++++++++++++++> Slicing: Set Slice[%d], index=%d, pos=%.2f,
# " % ( iAxis, sliceIndex, p1[0] ), textDisplay
                self.slicePosition[iAxis] = sliceIndex
                self.updateTextDisplay(textDisplay)
