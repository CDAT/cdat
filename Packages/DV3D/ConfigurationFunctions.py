...
def deserialize(data_obj):
    ...
    if isinstance(data_obj, dict):
        ...
        for item in data_obj.items():
            rv[deserialize(item[0])] = deserialize(item[0])
        return rv
...
class PlotType:
    ...
    def isLevelAxis(cls, pid):
        ...
        if (lname.find('lev') >= 0):
            return True
        if (lname.find('bottom') >= 0) and (lname.find('top') >= 0):
            return True
        if pid in cls.LevelAliases:
            return True
        if lname in cls.LevelAliases:
            return True
        return False
...
def get_parameter_cell(serialized_value):
    namelist = deserialize_address(serialized_value)
    if (len(namelist) < 2) or (len(namelist[1]) < 3):
        return None
...
# Parameter: ', param_name, " = ", str( data )
    ...
    def getConfigurableFunction(self, name, **args):
        ...
        if rv is None:
            ...
            if type == ConfigurableFunction.Default:
                rv = ConfigurableFunction(self, name, **args)
            elif type == ConfigurableFunction.Slider:
                rv = ConfigurableSliderFunction(self, name, **args)
            else:
    ...
    def getStateData(self):
        ...
        for cf in self.configurableFunctions.values():
            ...
            if state_data_elem:
                state_data.append(state_data_elem)
        return state_data

    def serializeState(self, asList=False):
        state_value = self.value.serializeState()
        state_data = None if ((state_value is None) or not self.persist) else [self.name, state_value]
        if state_data and not asList:
            state_data = "=".join(state_data)
        return state_data
...
#        self.scaling_bounds = None
    ...
    def restoreState(self, stateData):
        ...
        elif isinstance(stateData, dict):
            state = stateData
        else:
    ...
    def unpack(self, value_strs):
        if len(value_strs) != len(self.values.keys()):
            print>>sys.stderr, " Error: parameter structure mismatch in %s ( %d vs %d )" % (
                self.name, len(value_strs), len(self.values.keys()))
            sys.stderr.flush()
        for (key, str_val) in zip(self.valueKeyList, value_strs):
            self.values[key] = deserialize_value(str_val)
        if self.debug:
            print " && Unpack parameter %s: %s " % (self.name, str(self.values))
    ...
    def __setitem__(self, key, value):
        ...
        if self.debug:
            print "Parameter[%s]: set value item[%s]: %s " % (self.name, key, str(value))
    ...
    def __call__(self, **args):
        self.values.update(args)
        if self.debug:
            print " && Update parameter %s: %s " % (self.name, str(self.values))
        args1 = [self.ptype]
    ...
    def initialize(self, config_str):
        ...
        if self.debug:
            print " && initialize parameter %s: %s " % (self.name, str(self.values))
        self.sort()
    ...
    def getInitValue(self, default_value=None):
        ...
        if ival is None:
            ival = self.getValues()
            if len(ival) == 0:
                ival = self.parent.getInitValue() if (self.parent is not None) else None
                if (ival is None):
                    return default_value
        return ival
...
#        self.group = args.get( 'group', ConfigGroup.Display )
    ...
    def serializeState(self):
        ...
        return None if ((state_data is None) or not self.persist) else "=".join([self.name, state_data])

    def restoreState(self, stateData):
        self.value.restoreState(stateData)

    def sameGroup(self, config_fn):
        if id(self) == id(config_fn):
            return False
        if self.name == config_fn.name:
            return True
        if (self.group is not None) and (self.group == config_fn.group):
            return True
        return False
...
class ConfigurableSliderFunction(ConfigurableFunction):
    ...
    def getSliderBounds(self):
        if self._slider_bounds is None:
...
#         return image_data
    ...
    def clipInput(self, extent):
        ...
        if vtk.VTK_MAJOR_VERSION <= 5:
            ...
            self.clipper.AddInputData(self._input)
    ...
    def getWorldCoords(self, image_coords):
        ...
        try:
            if plotType == 'xyt':
                ...
                world_coords = [getFloatStr(lon[image_coords[0]]), getFloatStr(
                    lat[image_coords[1]]), getFloatStr(lev[image_coords[2]])]
        except:
...
#                 if lev_ordering == 'down': image_coord = - ( image_coord + 1 )
    ...
    def getDataRangeBounds(self):
        ...
        if self.rangeBounds:
            ...
            if (len(self.rangeBounds) > 2):
                srange.append(self.rangeBounds[2])
            else:
                srange.append(0)
        else:
            srange = [0, 0, 0]
...
# print "\n *****************  GetImageValues: data_values = %s, range =
# %s, imageValues = %s **************** \n" % ( str(data_value_list),
# str(self.scalarRange), str(imageValues) )
        return imageValues
        ...
        if self.fieldData:
            ...
            if plotIndex < len(mdList):
                ...
                try:
                    ...
                    pass
        print>>sys.stderr, "[%s]: Error, Metadata for input %d not found in ispec[%x]  " % (
            self.__class__.__name__, plotIndex, id(self))
        return {}
...
def extractMetadata(fieldData):
    ...
    if varlist is None:   # module.getFieldData()
