...
class MultiVarPointCollection():
    ...
    def getDataBlock(self, var):
            ...
            if iTimeIndex > 0:
                ...
                if self.lev is None:
                    if len(var.shape) == 1:
                        ...
                        np_var_data_block = var[:, self.istart::self.istep].data
                        np_var_data_block = np_var_data_block.reshape(
                            [np_var_data_block.shape[0] * np_var_data_block.shape[1], ])
                    self.nLevels = 1
                else:
        ...
        if not isNone(np_var_data_block):
            if self.missing_value:
                np_var_data_block = numpy.ma.masked_equal(np_var_data_block, self.missing_value, False).flatten()
            else:
    ...
    def processCoordinates(self, lat, lon):
        # print "Process Coordinates, lat = %s%s, lon = %s%s " % ( lat.id,
        # str(lat.shape), lon.id, str(lon.shape)  )
        ...
        if self.istep <= 0:
            self.istep = max(self.n_input_points / self.max_points, 1)
        if lon.__class__.__name__ == "TransientVariable":
            self.lat_data = lat.flatten()[self.istart::self.istep] if (
                self.point_layout == PlotType.List) else lat.flatten()[::]
        ...
        if (self.point_layout == PlotType.List) and (self.roi is not None):
    ...
    def setPointHeights(self, **args):
        ...
        if self.lev is None:
            ...
            if isinstance(z_scaling, (list, tuple)):
                z_scaling = z_scaling[0]
    ...
    #                print " setPointHeights: zdata shape = %s " % str( zdata.shape ); sys.stdout.flush()
                    ...
                    if self.data_height is None:
                        self.data_height = (self.vertical_bounds[1] - self.vertical_bounds[0])
                    self.point_data_arrays['z'] = zdata * (stage_height / self.data_height)
                else:
                    print>>sys.stderr, "Can't find height var: %s " % height_varname
            else:
                if (z_scaling != self.z_scaling) or (self.roi is not None) or (
                        (height_varname != self.hgt_var) and (height_varname == 'Levels')):
                    self.z_scaling = z_scaling
                    if height_varname:
                        self.hgt_var = height_varname
                    np_points_data_list = []
                    zstep = stage_height / nz
# print "  -----> SetPointHeights: z_scaling=%s, stage_height=%s" % (
# str(z_scaling), str(stage_height) )
                    for iz in range(nz):
                        zvalue = iz * zstep
                        if self.point_layout == PlotType.List:
                            z_data = numpy.empty(self.lon_data.shape, self.lon_data.dtype)
                        ...
                        z_data.fill(zvalue)
                        if ascending:
                            np_points_data_list.append(z_data.flat)
                        else:
                            np_points_data_list.insert(0, z_data.flat)
#                    print " **** Sample z data value: %s" % str( np_points_data_list[nz/2][10] )
    ...
    def getAxisBounds(self, axis=None):
        return self.getBounds() if (axis is None) else self.axis_bounds[axis]
        ...
        if lev is None:
            for axis_spec in var.getDomain():
                ...
                if axis.id in lev_aliases:
                    axis.designateLevel()
                    return grid_lev if (grid_lev is not None) else axis
            for axis_spec in var.getDomain():
                axis = axis_spec[0]
...
#         return var   # Subset later
    ...
    def initialize(self, args, **cfg_args):
        ...
        self.grid_vars = grd_vars if (grd_vars is not None) else self.df.variables[0]
    ...
    def initPoints(self, var_proc_op=None, z_scale=0.5):
        ...
        if not (isNone(lat) or isNone(lon)):
            ...
            self.metadata['lat'] = (
                getattr(
                    lat, 'long_name', 'Latitude'), getattr(
                    lat, 'units', None), self.axis_bounds.get(
                    'y', None))
            self.vars['lon'] = lon
            self.metadata['lon'] = (
                getattr(
                    lon, 'long_name', 'Longitude'), getattr(
                    lon, 'units', None), self.axis_bounds.get(
                    'x', None))
            ...
            if self.lev is not None:
                self.metadata['lev'] = (self.lev.__dict__.get('long_name', self.lev.id),
                                        self.lev.units, self.axis_bounds.get('z', None))
...
# print "MultiVarPointCollection.execute: %s, mask range = %s  " % ( str(
# args ), str( (vmin, vmax) ) ); sys.stdout.flush()
                    if isNone(threshold_mask):
                        self.thresholdTargetType = 'coords' if var_op[0] in [
                            'lat', 'lon', 'lev', 'x', 'y', 'z'] else 'vardata'
