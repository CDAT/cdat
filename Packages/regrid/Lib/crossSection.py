import Numeric, copy, MA, string, _regrid
...
class CrossSectionRegridder:
    ...
    def __init__(self, latIn, latOut, levIn, levOut, latTypeIn = None, latSizeIn = None,
        ...
        if calculateMean == 0:                                                   # meaningful grid
            ...
            bnout = Numeric.array([90.0], Numeric.Float32)
            bsout = Numeric.array([-90.0], Numeric.Float32)
    ...
    def rgrd(self, dataIn, missingValueIn, missingMatch, logYes = 'yes', positionIn = None, maskIn = None, missingValueOut = None):
        ...
        aout = Numeric.zeros(tuple(aoutList), Numeric.Float32)                      # memory for aout
        ...
        ap = Numeric.zeros(tuple(apList), Numeric.Float32)                      # memory for ap
        ...
        levIn = self.levIn[:].astype(Numeric.Float64)
        levOut = self.levOut[:].astype(Numeric.Float64)
...
def get_latitude_wts_bnds(checklatpass):
    ...
    if checklatpass[0] < checklatpass[nlat-1]:                         # need a copy?
        checklat = Numeric.array(checklatpass, Numeric.Float64)
        checklat = checklat[::-1]
...
def latitude_bounds(lat_bnds): 
    ...
    latbnds = lat_bnds.astype(Numeric.Float32)
...
def get_region_latitude_wts_bnds(latRegionpass, latType, latSize):
    ...
    if latRegionpass[0] < latRegionpass[nlat-1]:                         # need a copy?
        latRegion = Numeric.array(latRegionpass, Numeric.Float64)
...
def sectionmask(dataIn, positionIn, maskIn, missingValueIn, missingMatch):
    ...
    if missingValueIn != None:                                                                    # use value from the file
        ...
        if missingMatch == 'greater': 
            ...
            n = Numeric.add.reduce(Numeric.where(Numeric.greater(Numeric.ravel(dataIn), miss), 1, 0))  
            if n > 0:
                missingFlag = 1

        elif missingMatch == 'equal': 
            miss = missingValueIn
            n = Numeric.add.reduce(Numeric.where(Numeric.equal(Numeric.ravel(dataIn), miss), 1, 0))  
            if n > 0:
                missingFlag = 2
            ...
            if missingValueIn > 0:
                ...
                miss = 0.99*missingValueIn
            n = Numeric.add.reduce(Numeric.where(Numeric.less(Numeric.ravel(dataIn), miss), 1, 0))  
            if n > 0:
                missingFlag = 3 
    ...
    if maskIn == None:                                        # ---- need to generate the mask from scratch ----
        ...
        amskin = Numeric.ones(dataShape, Numeric.Float32)               # allocate memory 
    ...
    #----------- START USER MASK SECTION
        ...
        if numberMaskDim == 2:                              #----------------------- USER SUPPLIED MASK IS 2D
            ...
            if numberDataDim > 2:
                amskin = Numeric.resize(maskIn, dataShape)            #  replicate the mask to size of dataIn
    ...
    if missingFlag != 0:                                               # there is missing data in dataIn to add to amskin
        if missingFlag == 1:
            amskin = Numeric.where(Numeric.greater(dataIn, miss),0.0 ,amskin)  
        elif missingFlag == 2:
            amskin = Numeric.where(Numeric.equal(dataIn, miss),0.0 ,amskin)  
        elif missingFlag == 3:
            amskin = Numeric.where(Numeric.greater(dataIn, miss),0.0 ,amskin)  
...
def section(latvals, levvals):
    ...
    xsection = Numeric.zeros( (nlev,nlat), Numeric.Float32)     # memory
...
def rmserror(data1, data2):
    ...
    d1 = Numeric.ravel(data1)
    d2 = Numeric.ravel(data2)

    sq = (d1 - d2)*(d1 - d2)
    error = Numeric.sum(sq)/len(d1)
    rmserror =  Numeric.sqrt(error)
...
if __name__=='__main__':
    ...
    latIn = cdms.createUniformLatitudeAxis(90.0, 46, -4.0)
    levList = [10., 40., 75., 100., 150., 250., 350.,   500., 650.,  850., 1000.]                  # pick some levels
    levIn = cdms.createAxis(Numeric.array(levList, Numeric.Float64), id='level')
    latOut = cdms.createGaussianAxis(64)
    levList = [10., 30., 50., 70., 100., 200., 300., 400., 500., 700.,850., 1000.]         # pick some levels
    levOut = cdms.createAxis(Numeric.array(levList, Numeric.Float64), id='level')

    xregridf = CrossSectionRegridder(latIn, latOut, levIn, levOut)

    dataIn = section(latIn[:], levIn[:])                                              # make some artificial data
    var = cdms.createVariable(dataIn, axes=(levIn, latIn), attributes={'units':'N/A'}, id='test')
