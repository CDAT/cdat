...
#include "VisusStdInt.h" // required on windows for addAttribute() function with uint32_t
...
typedef struct XMLDLLENTRY XMLNode
{
  ...
  public:
    ...
    // ***************************************************************
    // VISUS Extensions!!!
    inline XMLAttribute  *addAttribute(XMLCSTR lpszName, int lpszValuev) {
      char temp[100];
      sprintf(temp, "%d", lpszValuev);
      return addAttribute(lpszName, temp);
    }

    inline XMLAttribute  *addAttribute(XMLCSTR lpszName, uint32_t lpszValuev) {
      std::stringstream ss;
      ss << lpszValuev;
      return addAttribute(lpszName, ss.str().c_str());
    }

    inline XMLAttribute  *addAttribute(XMLCSTR lpszName, long lpszValuev) {
      std::stringstream ss;
      ss << lpszValuev;
      return addAttribute(lpszName, ss.str().c_str());
    }

    inline XMLAttribute  *addAttribute(XMLCSTR lpszName, double lpszValuev) {
      char temp[1000];
      sprintf(temp, "%f", lpszValuev);
      return addAttribute(lpszName, temp);
    }

    inline XMLAttribute  *addAttribute(XMLCSTR lpszName, bool lpszValuev) {
      return addAttribute(lpszName, lpszValuev ? "1" : "0");
    }
    // ***************************************************************
...
template <class T>
inline void addChild(XMLNode& parent, const char* name, const T& values)
{
  std::stringstream ss;

  XMLNode sub = parent.addChild(name);
  for (typename T::const_iterator iter=values.begin(); iter!=values.end(); ++iter)
  {
    ss << (*iter) << " ";
  }
  sub.addText(ss.str().c_str());
}


template <class T>
inline void getChild(XMLNode& parent, const char* name, T& values)
{
  std::stringstream ss;
  XMLNode sub = parent.getChildNode(name);
  ss << sub.getText();

  for (typename T::iterator iter=values.begin(); iter!=values.end(); ++iter)
  {
    ss >> (*iter);
  }
}
...
inline bool xmltobool(const char* string, int defaultValue)
{
  return xmltoi(string, defaultValue) != 0 ? true : false;
}
/********************************************************************************* */
