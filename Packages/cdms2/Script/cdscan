...
def main(argv):
    ...
    try:
        args, lastargs = getopt.getopt(argv[1:],"a:c:d:e:f:hi:jl:m:p:qr:s:t:x:",["include=","include-file=","exclude=","exclude-file=","time-linear=","notrim-lat","var-locate=","ignore-open-error"])
    except getopt.error:
        print sys.exc_value
        print usage
        sys.exit(0)
    ...
    for extendPath in dsetargs:
        ...
        for axis in extendDset.axes.values():
            if not ((splitOnTime and (axis.isTime() or axis.id==timeid)) or (splitOnLevel and (axis.isLevel() or axis.id==levelid))):
    ...
    for path in fileargs:
        ...
        for varname in varnames:
            ...
            varentry = [None]*8         # [timestart, timeend, levstart, levend, path, timeid, levid, calendar]
            varentry[4] = basepath
            ...
            for axis, start, length, truelen in domain:
                if (splitOnTime and (axis.isTime() or axis.id==timeid)) or (splitOnLevel and (axis.isLevel() or axis.id==levelid)):
    ...
    for varlist, slicelist in varindex:
        slice0 = slicelist[0]
        a,b,c,d,path0,timename,levname,calen = slice0
    ...
    for varlist, varentry in varindex:
        ...
        timeproj = {}
        levproj = {}
        for time0, time1, lev0, lev1, path, timename, levname, calendar in varentry:
        ...
        if splitOnTime and timename is not None:
            ...
            axes.append((varlist,fullaxis,name,partition,timeCoordToInd,units,opartition,timelinCoordToInd, calendar))
    ...
    for vlist1, axis1, name1, partition1, coordToInd1, units1, opartition1, linCoordToInd1, calen1 in axes:
        for vlist2, axis2, name2, partition2, coordToInd2, units2, opartition2, linCoordToInd2, calen2 in axes2:
            if len(axis1)==len(axis2) and name1==name2 and partition1==partition2 and units1==units2 and numpy.ma.allclose(axis1,axis2)==1 and calen1==calen2:
                vlist2.extend(vlist1)
                break
        else:
            axes2.append((copy.copy(vlist1),axis1, name1, partition1, coordToInd1, units1, opartition1, linCoordToInd1, calen1))
    ...
    for varindexname, varindexvalue in varindex:
        ...
        newslicedict = {}
        for time0, time1, lev0, lev1, path, timename, levname, calendar in varindexvalue:
