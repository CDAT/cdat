...
"""
    ...
    def create_dialogs(self):
        ...
        self.template_selector.component('scrolledlist').component(
            'listbox').configure(selectmode='single')
        self.template_selector.component(
            'hull').transient(self.dialog.interior())
    ...
    def update_template_selector_list(self):
        ...
        choices = sorted(map(lambda x: x.strip(), choices))
    ...
    # The loops for updating, etc. are over the editors that *have* been
    # created.
...
""" % name)
    ...
    def refresh_data(self, attr_name, mem_name, value):
        # This function is need to flush the events. I don't undstanad why?
        a = dir(self.editors)
...
# print "I am here in the set", mem_name, value, i
    ...
    def saveas(self):
        ...
        if s in vcs.listelements('template'):
            showerror(
                'Error Message',
                'There already is a template named ' +
                s)
            return
...
""" % (os.environ['HOME'], dotdir,
    ...
    def ask_save(self):
        ...
        if self.is_changed() or self.is_changed_from_original():
            ...
            result = self.asksavedialog.activate(
                geometry="+%s+%s" %
                (g[0], g[1]))
            self.asksavedialog.withdraw()
            print 'did something'
    ...
    def ask_save_execute(self, name):
        ...
        # this return value used by callers to cancel op
        elif name == 'Cancel':
            self.asksavedialog.deactivate(name)
...
class CounterEditor (SingleEditor):
    ...
    def get(self):
        "Return value"
        return int(
            self.widget.component('entryfield').component('entry').get())

    def set(self, value):
        self.widget.component('entryfield').setentry(str(value))

    def color_field(self, color):
        self.widget.component('entryfield').component(
...
class FloatEditor (SingleEditor):
    ...
    def __init__(self, parent, name, editor):
        ...
        self.normalbackground = self.widget.component(
