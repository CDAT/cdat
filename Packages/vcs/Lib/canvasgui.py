...
class CanvasGUI:
    ...
    def __init__(self, canvas=None, top_parent=None, got_root=0):
        ...
        # an Tkinter.Image object, then the image is destroyed and thus not
        # displayed.
        ...
        try:
            self.top_parent.menu.vcs_canvas_gui_settings_flg
        except:
            self.called_from_vcdat = 0
        self.canvas = canvas
...
# DEAN        parent.protocol("WM_DELETE_WINDOW",
# gui_control.Command(self.close_VCS_Canvas, canvas))
...
#		label = 'Delete')
        ...
        self.notebook = Pmw.NoteBook(
            bigframe,
            raisecommand=gui_control.Command(
                self.evt_tab,
                parent,
                canvas))
        ...
        self.command_line_tab = command_line_tab_gui(
            self,
            page,
            parent,
            canvas)
...
#        page = self.notebook.add('Plot Options')
        ...
        if (got_root == 0):
            ...
            self.timelabel = Tkinter.Label(
                frame1,
                text="%s 1:1 [1 step]: " %
                (self.animated_dimension))
            self.timelabel.pack(side='left')

            self.framescale = Tkinter.Scale(
                frame1,
                orient='horizontal',
                troughcolor=winColor,
                from_=1,
                to=1)
            self.framescale.configure(
                command=gui_control.Command(
                    self.evt_framescale,
                    parent,
                    canvas))
            self.framescale.pack(
                side='left',
                fill='x',
                expand=1,
                padx=5,
            ...
            gui_support.balloon.bind(
                self.framescale,
                "Stop the animation first, then drag to select a time slice.")
           ...
           '''
            ...
            gui_support.balloon.bind(
                ...
                "Pause the time (1/10 sec.) between each frame while the animation is running.")

            #frame3b = Tkinter.Frame( frame3, background = winColor, borderwidth = 1, relief = 'sunken' )
            frame3b = Tkinter.Frame(
                frame3,
                background=winColor,
                borderwidth=1,
                relief='sunken')
            ...
            unzoom_img = Tkinter.PhotoImage(
                ...
                file=os.path.join(
                    ...
                    'bin',
                    'viewmag-.gif'))
            self.unzoom_btn = Tkinter.Button(
                frame3b, image=unzoom_img, command=gui_control.Command(
                    self.evt_zoom, parent, canvas, -1))
            self.unzoom_btn.pack(
                side='left',
                fill='x',
                expand=1,
                padx=0,
            ...
            pandown_img = Tkinter.PhotoImage(
                master=frame3b,
                file=os.path.join(
                    vcs.__path__[0],
                    '..',
                    ...
                    'bin',
                    'pan_down.gif'))
            self.pandown_btn = Tkinter.Button(
                frame3b, image=pandown_img, command=gui_control.Command(
                    self.evt_pan_vert, parent, canvas, -20))
            self.pandown_btn.pack(
                side='left',
                fill='x',
                expand=1,
                padx=0,
            ...
            panleft_img = Tkinter.PhotoImage(
                master=frame3b,
                file=os.path.join(
                    vcs.__path__[0],
                    '..',
                    ...
                    'bin',
                    'pan_left.gif'))
            self.panleft_btn = Tkinter.Button(
                frame3b, image=panleft_img, command=gui_control.Command(
                    self.evt_pan_hori, parent, canvas, -20))
            self.panleft_btn.pack(
                side='left',
                fill='x',
                expand=1,
                padx=0,
            ...
            panright_img = Tkinter.PhotoImage(
                master=frame3b,
                file=os.path.join(
                    vcs.__path__[0],
                    '..',
                    ...
                    'bin',
                    'pan_right.gif'))
            self.panright_btn = Tkinter.Button(
                frame3b,
                image=panright_img,
                command=gui_control.Command(
                    self.evt_pan_hori,
            ...
            img1 = Tkinter.PhotoImage(
                ...
                file=os.path.join(
                    ...
                    'bin',
                    '2leftarrow.gif'))
            self.firstframe = Tkinter.Button(
                frame2,
                image=img1,
                command=gui_control.Command(
                    self.evt_firstframe,
...
#        self.pause.configure(state = 'disabled')
            ...
            img5 = Tkinter.PhotoImage(
                file=os.path.join(
                    ...
                    'bin',
                    'player_play.gif'))
            self.playforward = Tkinter.Button(
                frame2,
                image=img5,
                command=gui_control.Command(
                    self.evt_play,
            ...
            img6 = Tkinter.PhotoImage(
                file=os.path.join(
                    ...
                    'bin',
                    'player_end2.gif'))
            self.stepforward = Tkinter.Button(
                frame2,
                image=img6,
                command=gui_control.Command(
                    self.evt_stepforward,
                    parent,
                    canvas))
            self.stepforward.pack(
                side='left',
                fill='x',
                expand=1,
                padx=5,
    ...
    def evt_save_plot_to_file(self, parent, canvas):
        ...
        filetypes = [
            ("Postscript File", ".ps"), ("GIF File", ".gif"), ("CGM File", ".cgm")]
        ...
        try:
            ...
            showerror(
                ...
                "There is an error with the output name. So nothing was saved.")
...
#      gui_busy.busyEnd( self, parent )
    ...
    def evt_page_orientation(self, parent, canvas, orientation):
        ...
        if orientation == 'portrait':
            w2 = int(self.top_parent.winfo_screenwidth() * .44)
            h2 = int(self.top_parent.winfo_screenheight() * 0.77)
            self.frame.configure(width=w2, height=h2)
            # Flush events without blocking the thread: could have used
            # "self.top_parent.update()"
            ...
            w2 = int(self.top_parent.winfo_screenwidth() * .55)
            h2 = int(self.top_parent.winfo_screenheight() * 0.54)
            self.frame.configure(width=w2, height=h2)
            # Flush events without blocking the thread: could have used
            # "self.top_parent.update()"
            self.top_parent.update_idletasks()
    ...
    def evt_tab(self, parent, canvas, text):
        ...
        if (text in ["Edit Plot"]):
            ...
            self.top_parent.busyWidgets = (
                parent,
                self.animation_speed.component('entry'))
            self.templateeditor = canvas.templateeditor(
                gui_parent=self.top_parent,
                canvas=canvas,
                template_name=plt.template,
                plot=plt,
                called_from=1)
...
#           _gui_template_editor.create(gui_parent=parent, canvas=canvas, plot=None, template_name='new2', template_orig_name='default')
            ...
            if last_tab == "Edit Plot":
               # DEAN            print "plot_kw = ", plot_kw
                slab1, slab2, tmpl, gtype, gname = self.show_plot_after_animation(
                    parent, canvas)

                # Save the changes to the plot
                try:
                    slab1.filename = plot_kw['file']
                except:
                    pass
                try:
                    slab1.source = plot_kw['source']
                except:
                    pass
                try:
                    slab1.id = plot_kw['dataname']
                except:
                    pass
                try:
                    slab1.long_name = plot_kw['title']
                except:
                    pass
                try:
                    slab1.units = plot_kw['units']
                except:
                    pass
                try:
                    slab1.getAxis(len(slab1.shape) - 1).id = plot_kw['xname']
                except:
                    pass
                try:
                    slab1.getAxis(len(slab1.shape) - 2).id = plot_kw['yname']
                except:
                    pass
                try:
                    slab1.comment1 = plot_kw['comment1']
                except:
                    pass
                try:
                    slab1.comment2 = plot_kw['comment2']
                except:
                    pass
                try:
                    slab1.comment3 = plot_kw['comment3']
                except:
                    pass
                try:
                    slab1.comment4 = plot_kw['comment4']
                except:
                    pass
                ...
                try:
                    # Update the saved animation slab to reflect the changes
                    canvas.update_animation_data(slab1)
                except:
                    pass

                canvas.update()
            try:
                canvas._SCREEN_DATA_FLAG()
            except:
                pass
            ...
            if (text in ["Animation"]):
                self.animated_dimension = canvas.return_dimension_info()[
                    'name'][-1]
                ...
                try:
                    self.show_data_plot_info(self, parent, canvas)
                except:
                    pass
...
# DEAN      print "1 ---  other = ", text
    ...
    def stop_animation(self, parent, canvas):
        try:
            ...
            if (self.animation_gui_first_time != 0):
                self.animation_gui.dialog.withdraw()
            self.evt_stop(self.top_parent, self.canvas, 0.5)
            slab1, slab2, tmpl, gtype, gname = self.show_plot_after_animation(
                self.top_parent, canvas)
            canvas.animate.close()
        ...
        #     print "plot_kw ************ = ", plot_kw
            ...
            if ((gtype in ['Vector', 'Scatter', 'Xvsy']) and (slab2 is None)):
                showerror(
                    "Error Message to User",
                    "Must have two variables to plot %s." %
                    self.new_gm)
                return
            if slab2 is None:
                vdpy = canvas.plot(slab1, tmpl, gtype, gname, sal=0)
            else:
                vdpy = canvas.plot(slab1, slab2, tmpl, gtype, gname, sal=0)
#             try:
#                temp_arg = []
...
# vdpy.array[0]="plot_1" # Do this so that the template will know which
# data to get
        except:
            pass
    ...
    def show_plot_after_animation(self, parent, canvas):
        ...
        try:
            if (self.called_from_vcdat == 1):  # Must be from VCDAT
                ...
                # Must not be from VCDAT, so do the except below
                error
        except:
            for x in aa:
                if cdms2.isVariable(aa[x]):
                    if ((cdms2.isVariable(aa[x])) and (canvas.varglist[0] is not None) and (
                            aa[x].name == canvas.varglist[0].name)):
                        slab1 = canvas.varglist[0]
                    if ((cdms2.isVariable(aa[x])) and (canvas.varglist[1] is not None) and (
    ...
    def display_variable_attributes(self, slab_list):
        ...
        gui_support.balloon.bind(
            self.name_btn,
            "Show more variable information.")
        self.scl2.window_create('end', window=self.name_btn)
        self.scl2.insert('end', '\n')   # end id

        for x in slab_list:
            if x[0:9] in ['long_name']:
                self.scl2.insert('end', (x + '\n'))  # long_name
        for x in slab_list:
            if x[0:5] in ['title']:
                self.scl2.insert('end', (x + '\n'))      # title
        for x in slab_list:
    ...
    def display_dimension_attributes(self, slab_list):
        ...
        for y in slab_list:
            if y[0:6] in ['   id:']:
                dim_id.append(y.strip())          # id
            if y[0:9] in ['   units:']:
                dim_units.append(y.strip())    # units
            if y[0:10] in ['   Length:']:
                dim_length.append(y.strip())  # Length
            if y[0:9] in ['   First:']:
                dim_first.append(y.strip())    # First
            if y[0:8] in ['   Last:']:
                dim_last.append(y.strip())      # Last
    ...
    def show_data_plot_info(self, parent, canvas):
        ...
        # Don't allow the user to modify the text
        self.scl2.configure(text_state='disabled')
    ...
    def show_animation_gui(self, parent, canvas):
        if ((self.animation_gui is None) and (
                self.animation_gui_first_time == 0)):
            self.animation_gui = canvas.animate.gui(gui_parent=self.top_parent)
            self.animation_gui_first_time = 1
        else:
            self.animation_gui.dialog.transient(
                self.top_parent)  # Keep widget on top of its parent
    ...
    def evt_framescale(self, parent, canvas, event):
        ...
        self.timelabel.configure(
            text="%s 1:%i [%i step]: " %
            (self.animated_dimension, self.nofs, self.frame_position))
    ...
    def evt_firstframe(self, parent, canvas):
        ...
        self.timelabel.configure(
            text="%s 1:%i [1 step]: " %
            (self.animated_dimension, self.nofs))
    ...
    def evt_lastframe(self, parent, canvas):
        ...
        self.timelabel.configure(
            text="%s 1:%i [%i step]: " %
            (self.animated_dimension, self.nofs, self.nofs))
    ...
    def evt_stop(self, parent, canvas, pause):
        if (self.create_flg == 1):
            ...
            # Timing issue. Let the animation thread catch up
            time.sleep(pause)
            self.timelabel.configure(
                text="%s 1:%i [%i step]: " %
                (self.animated_dimension, int(
    ...
    def update_end_of_animation_creation(self):
        ...
        # Slow down the animation so that threading can catch up with the GUI
        # updates.
        sf = dir(self)
        # self.top_parent.update_idletasks() # Flush events without blocking
        # the thread: could have used "self.top_parent.update()"
    ...
    def update_animation(self, frame_count):
        ...
        # updates.
        sf = dir(self)
        # self.self.top_parent.update_idletasks() # Flush events without
        # blocking the thread: could have used "self.top_parent.update()"
        self.timelabel.configure(
            text="%s 1:%i [%i step]: " %
            (self.animated_dimension, frame_count, frame_count))
    ...
    def evt_play(self, parent, canvas, direction):
        ...
        if (self.create_flg == 0):
            ...
            # Needed to prevent flashing and insure the first image is saved
            canvas.animate.run_flg = 1

            # Activate the other control buttons
            self.firstframe.configure(
                state='normal',
                command=gui_control.Command(
                    self.evt_firstframe,
                    parent,
                    canvas))
            self.stepback.configure(
                state='normal',
                command=gui_control.Command(
                    self.evt_stepback,
                    parent,
            ...
            self.stepforward.configure(
                state='normal',
                command=gui_control.Command(
                    self.evt_stepforward,
                    parent,
                    canvas))
            self.lastframe.configure(
                state='normal',
                command=gui_control.Command(
                    self.evt_lastframe,
                    parent,
...
#      self.txt.setentry( slab1.id )
    ...
    gui_support.balloon.bind(
        self.eny1,
        "Specify the desired X-axis major tick marks and labels.\nFor example:\nNone\n[ ] or { }\n[ 0, 10, 20 ]  or  { 0:'0', 10:'10', 20:'20' }\n[ 0, 10 ]  or  { 0:'text', 10:'more text' }")
    self.eny1.clear()
    try:
    ...
    gui_support.balloon.bind(
        self.eny2,
        "Specify the desired X-axis minor tick marks.\nFor example:\nNone\n[ ] or { }\n[ 0, 10, 20 ]  or  { 0:'0', 10:'10', 20:'20' }\n[ 0, 10 ]  or  { 0:'text', 10:'more text' }")
    self.eny2.clear()
    try:
    ...
    gui_support.balloon.bind(
        self.eny3,
        "Specify the desired Y-axis major tick marks and labels.\nFor example:\nNone\n[ ] or { }\n[ 0, 10, 20 ]  or  { 0:'0', 10:'10', 20:'20' }\n[ 0, 10 ]  or  { 0:'text', 10:'more text' }")
    self.eny3.clear()
    try:
    ...
    gui_support.balloon.bind(
        self.eny4,
        "Specify the desired Y-axis minor tick marks.\nFor example:\nNone\n[ ] or { }\n[ 0, 10, 20 ]  or  { 0:'0', 10:'10', 20:'20' }\n[ 0, 10 ]  or  { 0:'text', 10:'more text' }")
    self.eny4.clear()
    try:
...
def evt_template_text(self, parent, canvas, txt):
    ...
    from gui_support import gui_color
    # print "txt = ", txt
    attr_name, attr_str = gui_template_editor.get_slab_info(
        self.top_parent, canvas, string.lower(txt), "Axis")
...
def show_template_gui(self, parent, canvas):
    # Show the template editor GUI
    self.templateeditor.dialog.dialog.deiconify()
...
def primitive_tab_gui(self, page, parent, canvas):
    ...
    gui_support.balloon.bind(
        self.prm_colormap,
        "Display the VCS Colormap Editor.")
...
def data_plot_info_tab_gui(self, page, parent, canvas):
    ...
    gui_support.balloon.bind(
        self.scl2,
        "Scroll window to view the Data and Plot information.")
...
def graphics_method_tab_gui(self, page, parent, canvas):
    ...
    self.select_graphics_method.pack(
        side='left',
        fill='both',
        expand=1,
        padx=2,
        pady=2)
    gui_support.balloon.bind(
        self.select_graphics_method,
        "Select a graphics method.")
...
#      gui_support.balloon.bind( self.gm_rename, "Rename the graphics method.")
    ...
    gui_support.balloon.bind(
        self.gm_save,
        "Save the graphics method to a file.")
...
def evt_change_graphics_method(self, parent, canvas):
    ...
    self.select_graphics_method.configure(
        label_text='Select the Graphics Method\nGraphics Method selected: ' +
        self.new_gm)
    self.select_gm_name.configure(
        label_text='Select Graphics Method name\nSelected name: ' +
        dpy_plot.g_name)
    ...
    for x in aa:
        if cdms2.isVariable(aa[x]):
            ...
            if ((cdms2.isVariable(aa[x])) and (canvas.varglist[1] is not None) and (
                    aa[x].name == canvas.varglist[1].name)):
                slab2 = canvas.varglist[1]
# canvas.clear() This was here before I started modifying may need to put
# back
    ...
    if (canvas.SCREEN_MODE() in ["TEDITOR"]):
        ...
        str_txt = self.txt.get()
        lbl_txt = self.txt.cget("label_text")
        template_priority = "self.templateeditor.new_template." + \
            string.lower(lbl_txt) + ".priority=1"
        exec(template_priority)
        canvas.plot_annotation(
            string.lower(lbl_txt),
            str_txt,
            self.templateeditor.plot.array[0])
        self.templateeditor.toggle_on_off_for_canvas(
            self.templateeditor,
            string.lower(lbl_txt))
    else:
        ...
        canvas.change_display_graphic_method(
            selected_display,
            self.new_gm,
            gm_name)
        # Clear the canvas and replot with updated data
...
#      result = dialog.activate(globalMode = 'nograb', geometry= "+%d+%d" % (d1, d5) )
    ...
    if result == 'OK':
        ...
        if new_name in canvas.listelements(self.new_gm):
            gui_message.error(
                'Dean - Error occurred while trying to copy %s graphics method %s to %s. Make sure the new graphics method name does not already exist.' %
                (self.gm_name, gm_selected, new_name))
            return
        try:
            ...
            gui_message.error(
                'Naomi - Error occurred while trying to copy %s graphics method %s to %s. Make sure the new graphics method name does not already exist.' %
                (self.new_gm, self.gm_name, new_name))
            return
...
#      result = dialog.activate(globalMode = 'nograb', geometry= "+%d+%d" % (d1, d5) )
    ...
    if result == 'OK':
        ...
        if new_name in canvas.listelements(self.new_gm):
            gui_message.error(
                'Error occurred while trying to copy %s graphics method %s to %s. Make sure the new graphics method name does not already exist.' %
                (self.gm_name, gm_selected, new_name))
            return
        try:
            if string.lower(self.new_gm) == 'boxfill':
...
def script_graphics_method(self, parent, canvas):
    ...
    if save_dialog == '':
        return
    if (save_dialog[-3:] != '.py') and (save_dialog[-4:] != '.scr'):
        save_dialog += '.py'
...
def command_line_tab_gui(self, page, parent, canvas):
    ...
    self.shell.component('entry').bind(
        "<Return>",
        gui_control.Command(
            evt_shell_command,
            self,
    ...
    self.shell.bind_all(
        "<Up>",
        gui_control.Command(
            evt_prevhist_command,
            self,
            parent))
    self.shell.bind_all(
        "<Down>",
        gui_control.Command(
            evt_nexthist_command,
            self,
            parent))
    gui_support.balloon.bind(
        self.shell,
...
def evt_shell_command(self, parent, event):
    ...
    try:
        # Redirect stdout and stderr to Shell output window
        sys.stdout = sys.stderr = Redirect_Output(self.scl1)
        ...
        print '***', exc_type_name + ':', sys.exc_info()[1]
    ...
    try:
        gui_defined_variables.update_defined()  # Must be from VCDAT
    except:
        pass
