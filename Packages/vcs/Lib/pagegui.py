...
class PageDescriptionEditor:
    ...
    def __init__(self, canvas=None, gui_parent=None, continents=None):
        ...
        group_bins = Pmw.Group(
            FRAME2,
            tag_text='Select Template, Graphics Method, and Data')
...
class create_page_description_panel:
    ...
    def create_form(self, template='default', graphics_method='Boxfill',
        ...
        gui_support.balloon.bind(
            form.lblc,
            "Set VCS Canvas ID from main VCDAT 'Plot Output\nDestination' choice button.")
        ...
        form.priority = Pmw.EntryField(form.line,
                                       ...
                                       validate=None,
                                       command=P_Command(
                                           self.evt_change_priority,
                                           id)
                                       )
        form.priority.pack(side='left', padx=5)
        gui_support.balloon.bind(
            form.priority,
            "Enter priority level for plot.")
        ...
        gui_support.balloon.bind(
            form.lbl,
            "Select button to remove or redraw plot.")
        ...
        form.template.component('entry').bind(
            '<ButtonPress>',
            P_Command(
                self.evt_replace_template,
                1,
                id))
        form.template.component('entry').bind(
            '<ButtonRelease>',
        ...
        gui_support.balloon.bind(
            form.template,
            "Set entry from green 'Template' section below.")
        ...
        gui_support.balloon.bind(
            form.gm,
            "Set entry from blue 'Graphics Method' section below.")
        ...
        form.data1.component('entry').bind(
            '<ButtonPress>',
            P_Command(
                self.evt_replace_data,
                1,
                id))
        form.data1.component('entry').bind(
            '<ButtonRelease>',
        ...
        gui_support.balloon.bind(
            form.data1,
            "Set entry from red 'Data' section below.")
...
#        form.data3.component('entry').bind('<KeyPress>', P_Command(self.evt_replace_data,0,id))
    ...
    def evt_change_priority(self, id):
        ...
        if ((isinstance(p_val, types.IntType))
                and (self.form[id].display is not None)):
    ...
    def evt_status_button(self, id):
        if (self.form[id].display is not None):
            if self.form[id].display.off == 0:
                ...
                if contout is None:
                    if xdim >= 0 and ydim >= 0 and tv.getAxis(
                            xdim).isLongitude() and tv.getAxis(ydim).isLatitude():
                        contout = 1
                    else:
                        contout = 0
                if (xdim >= 0 and ydim >= 0 and (
                        contout >= 1) and (contout < 12)):
    ...
    def do_plot(self, id):
        ...
        if string.lower(gm_type) in [
                'boxfill', 'isofill', 'isoline', 'outfill', 'outline', 'scatter', 'vector']:
            gm_min_size = 2
        ...
        if plot_flg == 5:
            ...
            self.form[id].display = vcs.plot(
                __main__.__dict__[data1],
                template,
                gm_type,
                gm,
...
class show_bin:
    ...
    def graphics_method(self, vcs, gm_name):
        ...
        self.gmain_menu.addmenu(
            'Graphics Method        ',
            "Graphics Method Options/Preferences",
            tearoff=1)
        #
        ...
        for x in graphics_method_list:
            self.gmain_menu.addmenuitem('Select Graphics Method', 'command', 'View graphics methods',
                                        label=x,
                                        command=P_Command(
                                            self.evt_select_graphics_method,
                                            x,
                                            vcs)
                                        )
