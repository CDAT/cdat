...
class ProjGUI:
    ...
    def __init__(self, canvas=None, gui_parent=None,
                 dialog_parent=None, master=None, name='default', Parent=None):
...
# self.ftype.pack(side='top')
        projs = sorted(vcs.listelements('projection'))
...
##         self.loaddialog.setentry( name )
    ...
    def setgui(self, *crap):
        ...
        if self.projection.name in [
                'default', 'robinson', 'polar', 'mollweide', 'linear']:
    ...
    def rename_proj(self, result):
        ...
        if result == 'OK':
            ...
            self.projection = self.canvas.createprojection(
                newname,
        ...
        if self.orig != []:
            try:
                for orig in self.orig[::-1]:
                    ...
                    if not self.projection.name in [
                            'default', 'robinson', 'polar', 'mollweide', 'linear']:
