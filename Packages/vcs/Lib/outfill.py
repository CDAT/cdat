...
"""
    ...
    def _setfillareaindex(self, value):
        if value is not None:
            value = VCS_validation_functions.checkIndex(
                self,
                'fillareaindex',
                value)
    ...
    def _setoutfill(self, value):
        if (type(value) in (ListType, TupleType, IntType, FloatType)):
            ...
            if (len(value) > 10):
                raise ValueError(
                    'The outfill attribute must have less than 10 values.')
            else:
                self._outfill = value
                setmember(self, 'outfill', value)  # update the plot
        else:
            raise ValueError(
                'The outfill attribute must be a tuple or list of values.')
    ...
    def _getdatawc_x1(self):
        if getmember(self, '_tdatawc_x1'):
            return cdtime.reltime(self._datawc_x1, self.datawc_timeunits).tocomp(
    ...
    def _getdatawc_x2(self):
        if getmember(self, '_tdatawc_x2'):
            return cdtime.reltime(self._datawc_x2, self.datawc_timeunits).tocomp(
                self.datawc_calendar)
        else:
    ...
    def _getdatawc_y1(self):
        if getmember(self, '_tdatawc_y1'):
            return cdtime.reltime(self._datawc_y1, self.datawc_timeunits).tocomp(
                self.datawc_calendar)
        else:
    ...
    def _getdatawc_y2(self):
        if getmember(self, '_tdatawc_y2'):
            return cdtime.reltime(self._datawc_y2, self.datawc_timeunits).tocomp(
                self.datawc_calendar)
        else:
    ...
    def __init__(
            self, parent, Gfo_name=None, Gfo_name_src='default', createGfo=0):
        #                                                         #
        ###########################################################
        ...
        self.__doc__ = self.__doc__ % (
            xmldocs.graphics_method_core, xmldocs.fillareadoc, xmldocs.outfilldoc)
...
"""
        ...
        if (scr_type == '.scr'):
            ...
            fp.write(
                "#----------Outfill (Gfo) member (attribute) listings ----------\n")
