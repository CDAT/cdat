...
def create(gui_parent=None, table='default',
           orientation='default', canvas=None, parent=None):
...
class FontGUI:
    ...
    def __init__(self, canvas=None, gui_parent=None, dialog_parent=None,
                 master=None, t_name='default', o_name='default', Parent=None):
        ...
        if self.text is None:
            if not t_name in vcs.listelements('texttable'):
                ...
                raise 'Error ' + o_name + \
                    ' is not a valid textorientation name'
            else:
...
##########################################################################
        ...
        title = 'Text Editor - table: ' + self.text.Tt_name + \
            ', orientation: ' + self.text.To_name
...
# self.ftype.pack(side='top')
        ...
        tables = sorted(vcs.listelements('texttable'))
        ...
        orientations = sorted(vcs.listelements('textorientation'))
...
# b0.pack(side='left')
    ...
    def setgui(self):
        ...
        if self.text.To_name in [
                'default', 'defcentup', 'defcentdown', 'defright', 'defcenter']:
...
# self.parent.properties_orientation.configure(scrolledlist_items=tables)
        self.parent.properties_orientation.component(
            'scrolledlist').setlist(orientations)
...
# print 'Should be changing the dialog now !',self.projection.name
    ...
    def rename_orientation(self, result):
        ...
        if result == 'OK':
            ...
            text = self.canvas.createtextorientation(
                newname,
                self.text.To_name)
# print 'Should be changing the dialog now !',self.projection.name
