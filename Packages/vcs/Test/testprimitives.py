...
def test():
    ...
    # Redraw a box around the drawing area, but use 'x.line' instead of
    # 'x.plot'.
    ln = x.getline('new')
    x.line(ln, bg=bg)
    support.check_plot(x)
    ...
    # x fill area positions
    fa.x = [[0.2, 0.5, 0.5, 0.2], [1.2, 1.8, 1.8, 1.2]]
    # x fill area positions
    fa.y = [[0.2, 0.2, 0.5, 0.5], [1.2, 1.2, 1.8, 1.8]]
    fa.color = 241
    x.plot(fa, bg=bg)
    support.check_plot(x)
