...
import selectors
import copy
from regrid import Regridder, PressureRegridder, CrossSectionRegridder
...
class AbstractVariable(CdmsObj, Slab):
    ...
    def __array__ (self, t=None):  #Numeric, ufuncs call this
...
# A child class may want to override this
    ...
    def getSlice (self, *specs, **keys):
        ...
        return self.subSlice(*specs, **keys)
    ...
    def _single_specs (self, specs):
        ...
        while i < nsupplied:
            if specs[i] is Ellipsis:
                ...
                if type(specs[i]) is types.IntType:
                    singles.append(j)
                j = j + 1
            i = i + 1
        return singles
    ...
    def specs2slices (self, speclist, force=None):
        ...
        for i in range(self.rank()):
            key = speclist[i]
            if type(key) is types.IntType:  # x[i]
    ...
    def _decodedType(self):
        ...
        if hasattr(self, 'scale_factor') and type(self.scale_factor) is Numeric.ArrayType:
            result = self.scale_factor.typecode()
        elif hasattr(self, 'add_offset') and type(self.add_offset) is Numeric.ArrayType:
            result = self.add_offset.typecode()
        else:
            result = MA.Float32
        return result
    ...
    def __getitem__(self, key):
        if type(key) is types.TupleType:
            ...
            if type(key) is types.IntType and key>=len(self):
                raise IndexError, "Index too large: %d"%key
            speclist = self._process_specs([key], {})

        # Note: raw=0 ensures that a TransientVariable is returned
        return self.getSlice(numericSqueeze=1, raw=0, *speclist)
