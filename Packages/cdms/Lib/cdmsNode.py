...
class CdmsNode:
    ...
    def write(self,fd=None,tablevel=0,format=1):
        ...
        printLimit = MA.get_print_limit()
        MA.set_print_limit()            # Ensure that all Numeric array values will be printed
        if self.dtd:
        ...
        fd.write('</'+self.tag+'>')
        if format: fd.write('\n')
        MA.set_print_limit(printLimit)  # Restore original
...
class AxisNode(CdmsNode):
    ...
    def __init__(self, id, length, datatype=CdLong,data=None):
        assert type(length) is IntType, 'Invalid length: '+`length`
        assert type(datatype) is StringType, 'Invalid datatype: '+`datatype`
        assert datatype in CdDatatypes, 'Invalid datatype: '+`datatype`
        if data is not None: assert type(data) is Numeric.ArrayType, 'data must be a 1-D Numeric array'
...
class LinearDataNode(CdmsNode):
    ...
    def __init__(self, start, delta, length):
        ...
        assert type(length) is IntType, 'Invalid length argument: '+`length`
...
class AttrNode(CdmsNode):
    ...
    def getDatatype(self):
        if self.datatype:
            ...
            return CdString
        elif type(self.value) is FloatType:
            return CdDouble
        elif type(self.value) is IntType:
            return CdLong
        else:
