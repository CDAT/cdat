...
class AbstractHorizontalGrid(AbstractGrid):
    ...
    def checkConvex(self):
        ...
        from Numeric import zeros, where, less, logical_or, compress
    ...
    def fixCutCells(self, nonConvexCells, threshold=270.0):
        ...
        nonConvexCells: 1D Numeric array of indices of nonconvex cells, as returned from
          checkConvex.
        ...
        from Numeric import take, array
        ...
        lonb2 = take(lonb, nonConvexCells)
        latb2 = take(latb, nonConvexCells)
...
class AbstractCurveGrid(AbstractHorizontalGrid):
    ...
    def getMesh(self, transpose=None):
        ...
        if self._mesh_ is None:
            ...
            mesh = Numeric.zeros((self.size(),2,4),latbounds.typecode())
            if (transpose is not None) and (transpose[1]==0):
                latbounds = Numeric.transpose(latbounds, (1,0,2))
                lonbounds = Numeric.transpose(lonbounds, (1,0,2))
            mesh[:,LAT,:] = Numeric.reshape(latbounds,(self.size(),4))
            mesh[:,LON,:]  = Numeric.reshape(lonbounds,(self.size(),4))
            self._mesh_ = mesh
        return self._mesh_
    ...
    def writeScrip(self, cufile, gridTitle=None):
        ...
        clat = MA.filled(copy.copy(blat))
        clat.shape = (ngrid,4)
        clon = MA.filled(copy.copy(blon))
        clon.shape = (ngrid,4)
        ...
        griddims[:] = Numeric.array([nj,ni], Numeric.Int32)
    ...
    def toGenericGrid(self, gridid=None):
        ...
        cornerLat = MA.filled(copy.copy(blat))
        cornerLat.shape = (ngrid,4)
        cornerLon = MA.filled(copy.copy(blon))
    ...
    def getIndex(self):
        ...
        if self._index_ is None:
            latlin = Numeric.ravel(MA.filled(self._lataxis_))
            lonlin = Numeric.ravel(MA.filled(self._lonaxis_))
    ...
    def intersect(self, spec):
        ...
        fullmask = Numeric.ones(ni*nj)
        Numeric.put(fullmask, points, 0)
        fullmask = Numeric.reshape(fullmask, (ni,nj))
    ...
    def flatAxes(self):
        ...
        if self._flataxes_ is None:
            ...
            alatflat = Numeric.ravel(alat)
            alonflat = Numeric.ravel(alon)
...
def readScripCurveGrid(fileobj, dims, whichType, whichGrid):
    ...
    if hasattr(cornerLat, 'units') and string.lower(cornerLat.units)[0:6]=='radian':
        ...
        cornerLat = Numeric.reshape(cornerLat, boundsshape)
        cornerLon = Numeric.reshape(cornerLon, boundsshape)
    ...
    if vardict.has_key(gridMaskName):
        ...
        mask = Numeric.reshape(mask, gridshape)
    else:
        mask = None
        
    if vardict.has_key(gridCenterLatName):
        centerLat = Numeric.reshape(fileobj(gridCenterLatName), gridshape)
    ...
    if vardict.has_key(gridCenterLonName):
        centerLon = Numeric.reshape(fileobj(gridCenterLonName), gridshape)
