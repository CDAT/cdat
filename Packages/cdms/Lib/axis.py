...
def createGaussianAxis(nlat):
    lats,wts,bnds = regrid._regrid.gridattr(nlat,'gaussian')
...
def createEqualAreaAxis(nlat):
    lats,wts,bnds = regrid._regrid.gridattr(nlat,'equalarea')
    latBounds = Numeric.zeros((nlat,2),Numeric.Float)
    latBounds[:,0] = bnds[:-1]
    latBounds[:,1] = bnds[1:]
    lat = createAxis(lats,latBounds,id="latitude")
...
#mfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmfmf
    ...
    def getModuloCycle(self):
        ...
        if type(cycle) is Numeric.ArrayType:
            cycle = cycle[0]
...
class TransientAxis(AbstractAxis):
    ...
    def __init__(self, data, bounds=None, id=None, attributes=None, copy=0):
        ...
        if isinstance(data, AbstractAxis):
            ...
            if MA.getmask(data) is not None:
                raise CDMSError, \
        ...
        if persistent:
            ...
            if hasattr(self,'bounds'):
                ...
                if boundsid is None:
                    ...
                    boundName = boundsid
                boundVar = file.createVariable(boundName, cdmsNode.NumericToCdType.get(bounds.typecode()), (self,file._boundAxis_))
                # And link to self
