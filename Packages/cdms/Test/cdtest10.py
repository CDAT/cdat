...
uar = MA.arange(NYR*NMO*NLAT*NLON)
uar.shape = (NYR*NMO,NLAT,NLON)
u = cdms.createVariable(uar, id='u', axes = (time, g.getLatitude(), g.getLongitude()))
u.units = 'm/s'

var = uar + 100000.
v = cdms.createVariable(var, id='v', axes = u.getAxisList())
v.units = 'm/s'

tar = uar + 200000.
t  = cdms.createVariable(tar, id='t', axes = u.getAxisList())
t.units = 'K'

imo = 0
for year in range(2000,2006):
    f = cdms.open('cdtest10_uv_%d.nc'%year,'w')
    uchunk = u.subSlice((imo,imo+12))
    f.write(uchunk)
    vchunk = v.subSlice((imo,imo+12))
    f.write(vchunk)
    f.close()
    g = cdms.open('cdtest10_t_%d.nc'%year,'w')
    if year==2004:
...
uar2p = uar[18:30]
if not MA.allclose(uar2,uar2p): markError('u.getRegion() from 2001-7 to 2002-7 failed',uar2[0,0,0])
...
uar3p = uar[48:54]
if not MA.allclose(uar3,uar3p): markError('u.getRegion() from 2003-7 to 2004-7 failed',uar3[0,0,0])
