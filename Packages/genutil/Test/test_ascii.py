...
try:
    vars = genutil.ASCII.readAscii(cdat_info.get_sampledata_path()+'/testASCII.asc',header=5)
except:
...
try:
    vars = genutil.ASCII.readAscii(cdat_info.get_sampledata_path()+'/testASCII.asc',header=5,shape=shapes,ids=ids)
except Exception,err:
