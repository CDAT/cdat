...
class WK(object):
    ...
    def __init__(self, frequency=1, number_of_days=96,
                 shift=30, dosymetric=1, tkbar=0):
        """
        ...
        dosymetric : if we need to do the symetric/antisymetric separation
                                                   (default=1)
        tkbar: Do we want a tk bar or text bar when processing FFTs
                                                   (default=0, i.e no tk)
        """
        ...
        if tkbar:  # When doing FFTs, text bar or tkbar
            ...
            self.tkbar = None
        # Do we need to do the symetric/antisymetric treatment
        self.symetric = dosymetric
        return
    ...
    def _setfrequency(self, value):
        ...
        # Number of time points to shift by
        self._NShift = int(self.shift * value)
        # Number of time point per subdomain
        self._NTSub = int(self.number_of_days * value)
# print self.scm()
    frequency = property(
        _getfrequency,
        _setfrequency,
        None,
        "data frequency in samples per day (default=1, i.e daily)")
    ...
    def _setnumber_of_days(self, value):
        ...
        # Number of time point per subdomain
        self._NTSub = int(self.frequency * value)
# print self.scm()
    number_of_days = property(
        _getnumber_of_days,
        _setnumber_of_days,
        None,
        "length of time subdomains in days (default=96 days)")
    ...
    def _setshift(self, value):
        ...
        self._shift = value
        # Number of time points to shift by
        self._NShift = int(self.frequency * value)
    shift = property(
        _getshift,
        _setshift,
        None,
        "Number of days to shift subtimedomains by (default=30 days)")
    ...
    def process(self, data):
        ...
        if len(t) < self._NTSub:
            raise Exception(
                "Error your data must have at least %i time steps," +
                "adjust frequency (currently: %i/day) or number_of_days" +
                " (currently: %i processed at once) to reach that limit," +
                " or get more data" %
...
# print 'Wave numbers:',ss
        ...
        Power = numpy.zeros(
            (PP, self._NTSub + 1, NL + 1, len(lats)), numpy.float)

        # LOOP through time sub domains
        prev = 0  # initialize the scrolling bar
        for Pcount in range(PP):
            if PP > 1:
                prev = genutil.statusbar(
                    float(Pcount), PP - 1, prev=prev, tk=self.tkbar)
        ...
        Power.longname = \
            'Real power spectrum for the many different' +\
            'parts (i.e. over separate time divisions)'
...
# __call__=get
    ...
    def background(self, S, A, wavenumber_smoothing_windows=((1000, 10,),),
                   frequencies_smoothing_windows=((.1, 5), (.2, 10),
                                                  (.3, 20), (1000, 40))):
        """ return the background power
        Input:
        Power, with frequencies as dimension 0 and wave numbers as dim 1
        frenquencies_smoothing_windows: list of tuples of frequency/number of
                                        121 smoothing,
                                        frequencies must be increasing
        wavenumber_smoothing_windows:   list of tuples of wavenumber/number of
                                        121 smoothing,
                                        wavenumber must be increasing

        Output:
        background_power

        Usage:
        bg=background(power,wavenumber_smoothing=((1000,10)),
        frenquencies_smoothing_windows=((.1,5),(.2,10),(.3,20),(1000,40)))
        """
        id = S.id.split("_")[0]
        power = (S + A) / 2.
...
# print istart,iend+1
            for i in range(n):
                ...
                prev = genutil.statusbar(
                    float(counter), total, prev=prev, tk=self.tkbar)
                counter += 1
...
# print istart,iend+1
    ...
    def scm(self, a=None, b=None, iterations=200):
        ...
        for i in range(1, iterations):
            ...
            if m * i in Mlist:
                return m * i
        raise "Error Couldn't find a common multiple within the first " + \
            str(iterations)
...
def symetrick(slab, axis='y'):
    ...
    if isinstance(axis, type(1)):
        ...
        if axis == 'x':
            ...
            ax = slab.getAxis(axs)
    if ax is None:
        raise 'Error Axis: ' + str(axis) + \
            ' does not exists (getAxis returned None)'
...
def mask_data(power,
              wn_fq_couples=[[[13, 14],
                              [0.09375, 0.10416667, 0.114583333333,
                               0.125, 0.21875, 0.22916667, ]],
                             [[-14.00, -13.00, -12.00, -11.00, -10.00,
                               -9.00, -8.00, ],
                              [0.791666666667, 0.78125, 0.77083333]],
                             ]
              ):
    ...
    for s in [13, 14]:
        for fq in [0.09375, 0.10416667, 0.114583333333,
                   0.125, 0.21875, 0.22916667, ]:
...
def tapertozero(slab, nmi, nn, tp):
    ...
    for i in range(1, N + 1):
        ...
        if j <= 0 or j > nn:
            ...
            slab[i - 1] = slab[i - 1] * .5 * \
                (1. - numpy.cos((j - 1) * numpy.pi / float(tp)))
        elif j > (nn - tp) and j <= nn:
            slab[i - 1] = slab[i - 1] * .5 * \
                (1. - numpy.cos((nn - j) * numpy.pi / float(tp)))
    return
