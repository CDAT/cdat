...
/*	Process an ISOFILL display command.				*/

    int procDGfi(str,tok)
	...
	if (*tok != '(')
	  {
	   flushp(fpout);
	   err_warn(1,fperr,
		"Error (procDGfi) - not a proper token (%s%c).\n",str,*tok);
	   return 0;
	  }
	   ...
	   if (tokm != '=')
	     {
	      flushp(fpout);
	      err_warn(1,fperr,
		"Error (procDGfi) - not a proper token (%s%c%s%c).\n",
			str,*tok,strm,tokm);
	      return 0;
	     }
	   for (i=0; i < 5 && cmpncs(strm,DGfi_attr[i]) != 0; i++);
	   if (i == 5)
	     {
	      flushp(fpout);
	      err_warn(1,fperr,
	   ...
	   if (i == 0) {
	      ...
	      if (!isnum(strn) )
		{
	   ...
	   else if (i == 2)
	     {
	      ...
	      for (pg=&Gfi_tab;pg!=NULL&&cmpnbl(pg->name,g_name)!=0;
				pg=pg->next);
	      if (pg == NULL)
	        {
	         flushp(fpout);
	         err_warn(1,fperr,
		   "Error - isofill name doesn't exist (%s%c%s=%s).\n",
			str,*tok,DGfi_attr[2],g_name);
	         return 0;
	        }
	     }
	   else if (i == 3)
	     {
	      ...
	      for (pp=&Pic_tab;pp!=NULL&&
				cmpnbl(pp->name,p_name)!=0;pp=pp->next);
	      if (pp == NULL)
	        {
	         flushp(fpout);
	         err_warn(1,fperr,
		   "Error - picture element name doesn't exist (%s%c%s=%s).\n",
			str,*tok,DGfi_attr[3],p_name);
	         return 0;
	        }
	     }
	   ...
	   if (tokn != ',')
	     {
	      flushp(fpout);
	      err_warn(1,fperr,
		"Error (procDGfi) - not a proper token (%s%c%s%c%s%c).\n",
...
/*		When no name was given set it to "0".			*/
	...
	c=d_update("ISOFILL",0,name,priority,"-2",g_name,p_name,a);
	if (tokm == EOF) c=EOF;
	return c;
      }
