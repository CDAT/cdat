...
/*	Process an OUTLINE display command.				*/
	...
	if (*tok != '(')
	  {
	   flushp(fpout);
	   err_warn(1,fperr,
		"Error (procDGo) - not a proper token (%s%c).\n",str,*tok);
	   return 0;
	  }
	   ...
	   if (tokm != '=')
	     {
	      flushp(fpout);
	      err_warn(1,fperr,
		"Error (procDGo) - not a proper token (%s%c%s%c).\n",
			str,*tok,strm,tokm);
	      return 0;
	     }
	   for (i=0; i < 5 && cmpncs(strm,DGo_attr[i]) != 0; i++);
	   if (i == 5)
	     {
	      flushp(fpout);
	      err_warn(1,fperr,
	   ...
	   if (i == 0) {
	      ...
	      if (!isnum(strn) )
		{
	   ...
	   else if (i == 2)
	     {
	      ...
	      for (pg=&Go_tab;pg!=NULL&&cmpnbl(pg->name,g_name)!=0;pg=pg->next);
	      if (pg == NULL)
	        {
	         flushp(fpout);
	         err_warn(1,fperr,
		   "Error - contour name doesn't exist (%s%c%s=%s).\n",
			str,*tok,DGo_attr[2],g_name);
	         return 0;
	        }
	     }
	   else if (i == 3)
	   ...
	   if (tokn != ',')
	     {
	      flushp(fpout);
	      err_warn(1,fperr,
		"Error (procDGo) - not a proper token (%s%c%s%c%s%c).\n",
...
/*		When no name was given set it to "0".			*/
	...
	c=d_update("OUTLINE",0,name,priority,"-2",g_name,p_name,a);
	if (tokm == EOF) c=EOF;
	return c;
      }
