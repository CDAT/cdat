...
/*			Graphics fill isoline names.			*/

    extern char Gfi_attr[18][16];
...
/*		Set up a table entry for isofills.			*/

	if ((ptab=(struct gfi_tab *)malloc(sizeof(Gfi_tab))) == NULL)
	  {
...
/*			Set up the attribute structure for isofills.	*/

	if ( (pat=ptab->pGfi_attr=
	         (struct gfi_attr *) malloc(sizeof(struct gfi_attr))) == NULL)
	  {
	   err_warn(1,fperr,"Error memory for %s not found \n",str);
	   killGfi(ptab);
	   return 0;
	...
	while ( ((c=getsttk(strm,&tokm)) || c == 0) && tokm != ')')
	  {
	   if (c == EOF || tokm == EOF)
	     {
	      err_warn(1,fperr,
		"Error - EOF in graphic isofill attributes for (%s%c).\n",
						str,tok);
	      killGfi(ptab);
	      return 0;
	     }
	   ...
	   if (c == 0)
	     {
	      err_warn(1,fperr,
		"Error - not a graphic isofill attribute name (%s%c%s%c).\n",
		str,*tok,strm,tokm);
	      killGfi(ptab);
	      return 0;
	     }
...
/*		Assign the real coordinates for the display space.	*/
	      ...
	      while (c != ')')
		{
		 ...
		 for (k=0; k < STRMAX &&
			   ...
			   c != EOF; k++)
				strf[k]=c;
		 if (c == EOF) {killGfi(ptab); return EOF;}
		 strf[k]='\0';
...
/*			Check the isolfill table entry and move
			to the table if it's ok.			*/

    int chk_mov_Gfi (struct gfi_tab *gtab)

      {
	int i,k;
	struct fill_range *pfro,*pfrn,*pfri,*pfrk;
	struct gfi_attr *pat,*pan;
	struct gfi_tab *ptab,*ptb;
	struct display_tab *pd;
	int I[12];

	for(ptb=ptab=&Gfi_tab;
...
/*		Check if it's trying to overwrite the default.		*/
         ...
         if (ptab == &Gfi_tab)
	  {
	   err_warn(1,fperr,"Error - can't replace the default (Gfi_%s).\n",
				gtab->name);
	   killGfi(gtab);
	   return 0;
	  }
...
/*		Check for changes and move modified parameters.		*/
	...
	if (pat->missing != pan->missing) I[9]=1;
	...
	while (pfrn != NULL && pfro != NULL)
	  {
	   if (pfrn->id   != pfro->id   || pfrn->lev1 != pfro->lev1 ||
	       pfrn->lev2 != pfro->lev2 || 
	       strncmp(pfro->fill_name, pfrn->fill_name,16)  != 0) I[10]=1;
	   if ((pfrn->next == NULL && pfro->next != NULL) ||
	       (pfrn->next != NULL && pfro->next == NULL) ) I[10]=1;
	   pfro=pfro->next;
	   pfrn=pfrn->next;
	  }
...
/*		If the table entry is being modified and it's being
	...
	if (!Inactive && fpout != NULL) prtGfi(fpout,gtab);
	return 1;
      }

/*		Print the isofill graph attributes that are defined.	*/

    int prtGfi(FILE *fp,struct gfi_tab *ptab)
	...
	if ((pgi=ptab->pGfi_attr) != NULL)
	    {
	     ...
	     for (i=0;i < 9;i++,pc+=256)
	       {
		if (*pc != '\0')
			k+=fprintf (fp,"%s=%s,",Gfi_attr[i],pc);
		if (k > 60)
		  {
		   fprintf (fp,"\n   ");
		   k=3;
	     ...
	     fprintf (fp,"\n   %s(%g,%g,%g,%g),",
		Gfi_attr[9],pgi->dsp[0],pgi->dsp[1],pgi->dsp[2],pgi->dsp[3]);
	     fprintf (fp,"\n   %s(%d,%d,%d,%d),",
		Gfi_attr[17],pgi->idsp[0],pgi->idsp[1],pgi->idsp[2],pgi->idsp[3]);

	     if (pgi->xat[0] != '\0')
                fprintf (fp,"\n   %s=%s,",Gfi_attr[10],pgi->xat);
	     if (pgi->yat[0] != '\0')
                fprintf (fp,"\n   %s=%s,",Gfi_attr[11],pgi->yat);
	     if (pgi->timeunits[0] != '\0')
                fprintf (fp,"\n   %s=%s,",Gfi_attr[15],pgi->timeunits);
             fprintf (fp,"\n   %s=%g,",Gfi_attr[14],pgi->missing);
             fprintf (fp,"\n   %s=%d,",Gfi_attr[16],pgi->calendar);
