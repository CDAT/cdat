...
/*			Graphics Y(x) vs x names.			*/

    extern char GYx_attr[17][16];


    extern int Inactive;

/*	Process a graphics Y(x) vs x assignment.
	The string defining the name must be in str[] and the following
	token must be in tok.						*/

    int procGYxvx_name(str,tok)
...
/*		Set up a table entry for Y(x) vs x.			*/

	if ((ptab=(struct gYx_tab *)malloc(sizeof(GYx_tab))) == NULL)
	  {
...
/*			Set up the attribute structure for Y(x) vs x.	*/

	if ( (pat=ptab->pGYx_attr=
	         (struct gYx_attr *) malloc(sizeof(struct gYx_attr))) == NULL)
	  {
	   err_warn(1,fperr,"Error memory for %s not found \n",str);
	   killGYx(ptab);
	   return 0;
	...
	while ( ((c=getsttk(strm,&tokm)) || c == 0) && tokm != ')')
	  {
	   if (c == EOF || tokm == EOF)
	     {
	      err_warn(1,fperr,
		"Error - EOF in graphic Y(x) vs x attributes for (%s%c).\n",
						str,tok);
	      killGYx(ptab);
	      return 0;
	     }
	   ...
	   if (c == 0)
	     {
	      err_warn(1,fperr,
		"Error - not a graphic Y(x) vs x attribute name (%s%c%s%c).\n",
		str,*tok,strm,tokm);
	      killGYx(ptab);
	      return 0;
	     }
...
/*			Find which attribute is being assigned.		*/
	   ...
	   for (pc=&(pat->proj[0]), i=0;
		 i < 17 && cmpnbl(strm,GYx_attr[i]) != 0; i++)
	     {
	      if (i < 9) pc+=256;
	     }
...
/*		Store the Y(x) vs x range definitions.		*/
...
/*			Check the Y(x) vs x table entry and move
			to the table if it's ok.			*/

    int chk_mov_GYx (struct gYx_tab *gtab)

      {
	int i,k;
	struct gYx_attr *pat,*pan;
	struct gYx_tab *ptab,*ptb;
	struct display_tab *pd;
	int I[14];

	for(ptb=ptab=&GYx_tab;
...
/*		Check if it's trying to overwrite the default.		*/
         ...
         if (ptab == &GYx_tab)
	  {
	   err_warn(1,fperr,"Error - can't replace the default (GYx_%s).\n",
				gtab->name);
	   killGYx(gtab);
	   return 0;
	  }
...
/*	If it isn't modifying a Y(x) vs x table entry then add it.	*/
...
/*		If the table entry is being modified and it's being
	...
	for(pd=&D_tab; pd!=NULL; pd=pd->next)
	 if (cmpncs("Yxvsx",pd->type)==0 && strcmp(pd->g_name,gtab->name)==0)
	  {
	   if (I[0] || I[9])
	     {
	      pd->xt1_seg[3]=TRUE;
	...
	if (!Inactive && fpout != NULL) prtGYx(fpout,gtab);
	return 1;
      }

/*		Print the Y(x) vs x graph attributes that are defined.	*/

    int prtGYx(FILE *fp,struct gYx_tab *ptab)
	...
	if ((pgi=ptab->pGYx_attr) != NULL)
	    {
	     ...
	     for (i=0;i < 9;i++,pc+=256)
	       {
		if (*pc != '\0')
			k+=fprintf (fp,"%s=%s,",GYx_attr[i],pc);
		if (k > 60)
		  {
		   fprintf (fp,"\n   ");
		   k=3;
	     ...
	     fprintf (fp,"\n   %s(%g,%g,%g,%g),",
		GYx_attr[9],pgi->dsp[0],pgi->dsp[1],pgi->dsp[2],pgi->dsp[3]);
	     fprintf (fp,"\n   %s(%d,%d,%d,%d),",
		GYx_attr[16],pgi->idsp[0],pgi->idsp[1],pgi->idsp[2],pgi->idsp[3]);

             if (pgi->xat[0] != '\0')
                fprintf (fp,"\n   %s=%s,",GYx_attr[10],pgi->xat);
             if (pgi->yat[0] != '\0')
                fprintf (fp,"\n   %s=%s,",GYx_attr[11],pgi->yat);

	     if (pgi->timeunits[0] != '\0')
                fprintf (fp,"\n   %s=%s,",GYx_attr[14],pgi->timeunits);
             fprintf (fp,"\n   %s=%d,",GYx_attr[15],pgi->calendar);
	     fprintf (fp,
			"\n   %s=%s,%s=%s)\n",
			GYx_attr[12],pgi->lb,
			GYx_attr[13],pgi->mb);
