...
def check_plot(x):
    ...
    try:
        ...
        pass
    fgood = "Good/Good_%s_%i_bg_%i_%s.svg" % (
        sys.argv[0][:-3], counter, bg, version)
...
# print "right1:",right
                ...
                try:
                    if left[i] == right[i]:
                        ...
                        try:
                            ...
                            # 5% diff or cerysmall numbers
                            if apdiff < 5. or adiff < 1.e-3:
                                same += 1
                            else:
                                total_diff += len(left[i])
                        except Exception as err:
                            if left[i].find(
                                    "Encoding") > -1 or left[i].find("CharStrings") > -1 or left[i].find("FontName") > -1:
                                pass
                            else:
                                total_diff += len(left[i])
                            pass
        ...
        if pdiff > diff:
            cwd = os.getcwd()
            raise Exception(
                "Error on test plot number %i, different plot, Test file name: %s differs from good: %s %% diff is %f\n" %
                (counter, os.path.join(
                    cwd, fnmtest), os.path.join(
                    cwd, fgood), pdiff))
# elif pdiff>0.:
