...
/*		Set up a table entry for X(t) vs Y(t).			*/
	...
	while ( ((c=getsttk(strm,&tokm)) || c == 0) && tokm != ')')
	  {
	   if (c == EOF || tokm == EOF)
	     {
	      err_warn(1,fperr,
		"Error - EOF in graphic X(t) vs Y(t) attributes for (%s%c).\n",
						str,tok);
	      killGXY(ptab);
	      return 0;
	     }
	   ...
	   if (c == 0)
	     {
	      err_warn(1,fperr,
	      "Error - not a graphic X(t) vs Y(t) attribute name (%s%c%s%c).\n",
		str,*tok,strm,tokm);
	      killGXY(ptab);
	      return 0;
	     }
...
/*			Check the X(t) vs Y(t) table entry and move
...
/*	If it isn't modifying a X(t) vs Y(t) table entry then add it.	*/
...
/*		If the table entry is being modified and it's being
	...
	for(pd=&D_tab; pd!=NULL; pd=pd->next)
	 if (cmpncs("XvsY",pd->type)==0 && strcmp(pd->g_name,gtab->name)==0)
	  {
	   if (I[0] || I[9])
	     {
	      pd->xt1_seg[3]=TRUE;
